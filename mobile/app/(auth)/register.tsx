...
export default function RegisterScreen() {
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1 bg-white dark:bg-zinc-900">
      <ScrollView className="flex-1 px-6 pt-12">
        <TouchableOpacity onPress={() => router.back()} className="mb-6">
          <Ionicons name="arrow-back" size={24} color="#16a34a" />
        </TouchableOpacity>

        <Text className="text-3xl font-bold text-gray-900 dark:text-white mb-1">Create Account</Text>
        <Text className="text-base text-gray-500 dark:text-gray-400 mb-8">Join the CropChain network</Text>
        ...
        <View className="space-y-4 mb-6">
          ...
          <View>
            <Text className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Your Role</Text>
            <View className="flex-row flex-wrap gap-2">
              {ROLES.map((r) => (
                ...
                >
                  <Ionicons name={r.icon as any} size={18} color={role === r.key ? '#16a34a' : '#9ca3af'} />
                  <Text className={`ml-2 text-sm font-medium ${role === r.key ? 'text-primary' : 'text-gray-600 dark:text-gray-400'}`}>
                    {r.label}
                  </Text>
                </TouchableOpacity>
              ))}
        ...
        <TouchableOpacity onPress={handleRegister} disabled={isLoading}
          ...
          {isLoading ? <ActivityIndicator color="white" /> : <Text className="text-white font-semibold text-base">Create Account</Text>}
        </TouchableOpacity>
