...
export default function HomeScreen() {
  ...
  const statsCards = [
    { label: 'Active Batches', value: String(activeBatches), icon: 'layers' as const, color: '#16a34a' },
    { label: 'Pending Sync', value: String(pendingCount), icon: 'cloud-upload' as const, color: '#d97706' },
    { label: 'Alerts', value: String(alertCount), icon: 'notifications' as const, color: '#dc2626' },
  ];
          ...
          batches.slice(0, 3).map((batch) => (
            ...
            >
              <View className="flex-row justify-between items-center">
                <View>
                  <Text className="font-semibold text-gray-900 dark:text-white">{batch.crop}</Text>
                  <Text className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Batch: {batch.id}</Text>
                </View>
                <View className="flex-row items-center">
                  <View className={`w-2 h-2 rounded-full mr-1.5 ${batch.status === 'active' ? 'bg-green-500' : 'bg-amber-500'}`} />
                  <Text className="text-xs text-gray-500 dark:text-gray-400 capitalize">{batch.stage}</Text>
                </View>
              </View>
            </TouchableOpacity>
          ))
