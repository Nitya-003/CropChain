...
export default function ProfileScreen() {
  ...
  const menuItems = [
    ...
    { icon: 'shield-checkmark' as const, label: 'Biometric Lock', right: 'Toggle' },
  ...
  return (
    <View className="flex-1 bg-gray-50 dark:bg-zinc-900">
      ...
      <View className="mx-5 mb-6 bg-white dark:bg-zinc-800 rounded-2xl p-6 items-center shadow-sm">
        ...
        <Text className="text-xl font-bold text-gray-900 dark:text-white">{user?.name || 'Guest'}</Text>
        <Text className="text-gray-500 dark:text-gray-400 text-sm">{user?.email || 'Not signed in'}</Text>
        {user?.role ? (
          <View className="mt-2 px-4 py-1 bg-primary/10 rounded-full">
            <Text className="text-primary text-sm font-semibold capitalize">{user.role}</Text>
          </View>
        ) : null}
      </View>
      ...
      <View className="mx-5 mb-6 bg-white dark:bg-zinc-800 rounded-2xl shadow-sm">
        <TouchableOpacity className="flex-row items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-zinc-700">
          <View className="flex-row items-center">
            <Ionicons name={isDark ? 'moon' : 'sunny'} size={22} color="#16a34a" />
            <Text className="ml-3 text-gray-900 dark:text-white font-medium">Dark Mode</Text>
          </View>
          <TouchableOpacity
            onPress={toggleTheme}
        ...
        {menuItems.map((item, index) => (
          ...
          >
            <View className="flex-row items-center">
              ...
              <Text className="ml-3 text-gray-900 dark:text-white font-medium">{item.label}</Text>
            </View>
      ...
      {user?.walletAddress ? (
        <View className="mx-5 mb-6 bg-white dark:bg-zinc-800 rounded-2xl p-5 shadow-sm">
          <Text className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Connected Wallet</Text>
          <Text className="text-gray-900 dark:text-white text-sm font-mono">{user.walletAddress}</Text>
        </View>
      ) : null}
