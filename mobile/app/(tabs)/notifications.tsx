...
export default function NotificationsScreen() {
    ...
    const getIconForType = (type: string) => {
        switch (type) {
            ...
            case 'recall': return 'shield-half';
    ...
    const getIconColorForType = (type: string) => {
        switch (type) {
            ...
            case 'recall': return '#f43f5e'; // rose-500
            case 'sync': return '#3b82f6'; // blue-500
            case 'approval': return '#10b981'; // emerald-500
    ...
    const getBgColorForType = (type: string, read: boolean) => {
        ...
        switch (type) {
            case 'alert': return '#fffbeb'; // amber-50
            case 'recall': return '#fff1f2'; // rose-50
            default: return '#f0fdf4'; // primary-50 (emerald-50)
        }
    };

    const renderItem = ({ item }: { item: Notification }) => (
        <TouchableOpacity 
            style={[styles.notificationCard, { backgroundColor: getBgColorForType(item.type, item.read) }]}
    ...
    return (
        <SafeAreaView style={styles.container} edges={['bottom', 'left', 'right']}>
            <View style={styles.header}>
                ...
                <TouchableOpacity onPress={markAllAsRead} disabled={!notifications.some(n => !n.read)}>
                    <Ionicons name="checkmark-done" size={24} color={notifications.some(n => !n.read) ? "#10b981" : "#9ca3af"} />
                </TouchableOpacity>
            </View>
            ...
            <View style={styles.filterContainer}>
                ...
                <TouchableOpacity 
                    style={[styles.filterButton, filter === 'alerts' && styles.filterAlertActive]}
                    onPress={() => setFilter('alerts')}
                >
                    <Text style={[styles.filterText, filter === 'alerts' && styles.filterTextActive]}>Alerts</Text>
                </TouchableOpacity>
            </View>
