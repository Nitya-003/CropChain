...
1. Try sending the transaction again from the app.
...
1. Add a local network in MetaMask (once):
...
3. Import one of the test accounts from the Hardhat node:
...
1. Confirm it is running on `http://localhost:3001`:
...
4. After changing any env or config values, restart both frontend and backend dev servers.
...
1. In a separate terminal, deploy the contract:

```bash
npx hardhat run scripts/deploy.js --network localhost
```

1. Copy the contract address printed in the terminal after deployment.

2. Update `backend/.env`: set `CONTRACT_ADDRESS=0x...`
...
1. If the frontend also reads the contract address from an env variable, update the frontend `.env` too and restart the frontend dev server.
...
1. Copy the new address into `CONTRACT_ADDRESS` (and frontend env if needed).

2. Restart backend and frontend so they both use the updated address.
...
- Messages mention things like INFURA_URL, PRIVATE_KEY, MONGODB_URI, JWT_SECRET, or CONTRACT_ADDRESS.
...
1. Edit `backend/.env` and fill in the required values:
...
1. Make sure MongoDB is running if you are using the local database.
...
1. Watch the terminal output for any errors or warnings.

2. If the app uses env variables (like NEXT_PUBLIC_API_URL or contract address), make sure they are defined in the frontend `.env` file and restart the dev server after editing them.
