...
test.describe('CropChain Lifecycle E2E', () => {
  ...
  test('Farmer should be able to create a new batch and see it', async ({ page }) => {
    ...
    await page.route('**/batches*', async route => {
      if (route.request().method() === 'OPTIONS') {
        ...
        await route.fulfill({
          headers: corsHeaders,
          json: { success: true, data: { batches: [{ batchId: 'batch-001', cropType: 'Wheat', quantity: 100, currentStage: 'farmer' }] } }
        });
      }
    });
    ...
    const fs = require('fs');
    fs.writeFileSync('/Users/tanmayy/.gemini/antigravity-ide/brain/12dd0766-dd1c-42f1-be57-ff148d1de5ea/farmer-page.html', await page.content());
    await expect(page.getByRole('cell', { name: 'Wheat', exact: true }).first()).toBeVisible();
    await expect(page.getByRole('cell', { name: '100 kg', exact: true }).first()).toBeVisible();
  });
  ...
  test('Mandi should see batches ready for auction', async ({ page }) => {
    ...
    await page.route('**/batches*', async route => {
      if (route.request().method() === 'OPTIONS') {
        ...
        await route.fulfill({
          headers: corsHeaders,
          json: { success: true, data: { batches: [{ batchId: 'batch-001', farmerName: 'Test Farmer', cropType: 'Wheat', quantity: 100, currentStage: 'mandi' }] } }
        });
      }
    });
    ...
    await page.route('**/auctions*', async route => {
      if (route.request().method() === 'OPTIONS') {
        ...
        await route.fulfill({ headers: corsHeaders, json: { success: true, data: { auctions: [] } } });
      }
    });
  ...
  test('Retailer should see completed batches and be able to track journey', async ({ page }) => {
        ...
        await route.fulfill({
          ...
          json: {
            ...
            data: {
              ...
              user: { id: 'retailer1', name: 'Test Retailer', role: 'retailer' }
            }
          }
        });
      }
    ...
    await page.route('**/batches*', async route => {
      if (route.request().method() === 'OPTIONS') {
        ...
        await route.fulfill({
          headers: corsHeaders,
          json: { success: true, data: { batches: [{ batchId: 'batch-001', farmerName: 'Test Farmer', cropType: 'Wheat', currentStage: 'transport', quantity: 100 }] } }
        });
      }
    });
