...
import { Shield, AlertTriangle, CheckCircle, XCircle, Search, RefreshCw, AlertOctagon, Info } from 'lucide-react';
...
function IncidentsDashboard() {
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState<'incidents' | 'pending'>('incidents');
  ...
  const fetchData = async () => {
    ...
    try {
      if (activeTab === 'incidents') {
        ...
        const incidentBatches = allBatches.filter((b: any) => b.isRecalled || b.status === 'Flagged' || b.isSpoiled);
  ...
  const handleRequestRecall = async () => {
    if (!selectedBatchId || justification.length < 20) {
      toast.error('Please provide a valid Batch ID and Justification (min 20 characters).');
      return;
    }
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || 'Failed to create recall request.');
    } finally {
  ...
  const handleApproveReject = async () => {
    ...
    setIsActionLoading(true);
    try {
      await realCropBatchService.approveRequest(selectedRequestId, approvalDecision, approvalReason);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 py-8 px-4">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-border/40 pb-6">
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <h1 className="text-3xl font-bold tracking-tight text-foreground">Incident Management Center</h1>
            <p className="text-sm text-muted-foreground">Manage recalls, contaminations, and flagged batches.</p>
          </div>
        </div>
        <div className="flex gap-2">
          {user?.role === 'admin' && (
            <Button onClick={() => setShowRecallModal(true)} className="bg-red-600 hover:bg-red-700 text-white">
          ...
          {isLoading ? (
            ...
            <Table>
              ...
              <TableBody>
                {batches.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">No incidents found.</TableCell>
                  </TableRow>
                ) : (
                  batches.map(batch => (
                    <TableRow key={batch.batchId} className="border-b border-border/40 hover:bg-muted/30">
                      <TableCell className="py-4 px-6 font-mono text-xs">{batch.batchId}</TableCell>
                      <TableCell className="py-4 px-6">
                        {batch.isRecalled ? (
                          <Badge className="bg-red-100 text-red-800 hover:bg-red-200 border-red-200">RECALLED</Badge>
                        ) : batch.status === 'Flagged' ? (
                          <Badge className="bg-amber-100 text-amber-800 hover:bg-amber-200 border-amber-200">FLAGGED</Badge>
                        ) : batch.isSpoiled ? (
                          <Badge className="bg-orange-100 text-orange-800 hover:bg-orange-200 border-orange-200">SPOILED</Badge>
                        ) : (
                          <Badge variant="outline">UNKNOWN</Badge>
                        )}
                      </TableCell>
                      <TableCell className="py-4 px-6 capitalize font-medium">{batch.cropType}</TableCell>
                      <TableCell className="py-4 px-6">{batch.quantity} kg</TableCell>
                      <TableCell className="py-4 px-6 capitalize">{batch.currentStage}</TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
            ...
            <Table>
              ...
              <TableBody>
                {pendingApprovals.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-8 text-muted-foreground">No pending approvals.</TableCell>
                  </TableRow>
                ) : (
                  pendingApprovals.map(req => (
                    <TableRow key={req.requestId} className="border-b border-border/40 hover:bg-muted/30">
                      <TableCell className="py-4 px-6 font-mono text-xs">{req.requestId}</TableCell>
                      <TableCell className="py-4 px-6">
                        <Badge variant="outline" className="uppercase text-xs">{req.actionType.replace('batch_', '')}</Badge>
                      </TableCell>
                      <TableCell className="py-4 px-6 font-mono text-xs">{req.batchId}</TableCell>
                      ...
                      <TableCell className="py-4 px-6">
                        ...
                        {user?.role === 'admin' && (
                          <span className="text-xs text-muted-foreground">Awaiting Inspector</span>
                        )}
                      </TableCell>
                    </TableRow>
                  ))
      ...
      {showRecallModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-card w-full max-w-md rounded-xl p-6 shadow-2xl border border-border">
            <h2 className="text-xl font-bold mb-4 text-foreground">Request Batch Recall</h2>
            <div className="space-y-4">
              <div>
                <label className="text-sm font-semibold mb-1 block text-foreground">Batch ID</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="text-sm font-semibold mb-1 block text-foreground">Justification (min 20 chars)</label>
                <textarea 
                  value={justification} 
              ...
              <div className="flex justify-end gap-3 mt-6 pt-2">
                ...
                <Button onClick={handleRequestRecall} disabled={isActionLoading} className="bg-red-600 hover:bg-red-700 text-white">
      ...
      {showApprovalModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-card w-full max-w-md rounded-xl p-6 shadow-2xl border border-border">
            <h2 className="text-xl font-bold mb-4 text-foreground">Review Request</h2>
            <p className="text-xs font-mono text-muted-foreground mb-6 bg-muted p-2 rounded">{selectedRequestId}</p>
            <div className="space-y-4">
              <div>
                <label className="text-sm font-semibold mb-1 block text-foreground">Decision</label>
                <select 
              ...
              <div>
                <label className="text-sm font-semibold mb-1 block text-foreground">Reason / Notes</label>
                <textarea 
              ...
              <div className="flex justify-end gap-3 mt-6 pt-2">
                ...
                <Button onClick={handleApproveReject} disabled={isActionLoading} className="bg-primary hover:bg-primary/90 text-primary-foreground">
