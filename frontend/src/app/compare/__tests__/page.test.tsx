...
const { mockGetBatch, mockPush, mockSearchParams } = vi.hoisted(() => ({
  ...
  mockSearchParams: vi
    .fn()
    .mockReturnValue(new URLSearchParams("ids=BATCH-001,BATCH-002")),
}));
...
describe("ComparePage", () => {
  beforeEach(() => {
    ...
    mockSearchParams.mockReturnValue(
      new URLSearchParams("ids=BATCH-001,BATCH-002"),
    );
  });
  ...
  it("shows error state when no batches are selected", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("No batches selected for comparison"),
      ).toBeInTheDocument();
    });
  });
});
