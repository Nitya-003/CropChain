      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div className="text-left">
            <h1 className="text-2xl font-bold tracking-tight text-foreground">Compare Recommendations</h1>
            <p className="text-sm text-muted-foreground">Select up to two saved results to compare side‑by‑side.</p>
          </div>
        </div>

        {/* List of saved results */}
        {saved.length === 0 && (
          <p className="text-center text-muted-foreground">No saved results. Save a recommendation from the recommendation page first.</p>
        )}
            ...
            <Card key={idx} className="border border-border bg-card">
              ...
              <CardContent className="p-4">
                ...
                {item.result.alternatives?.length > 0 && (
                  <p>Alternatives: {item.result.alternatives.map((a: any) => a.crop).join(', ')}</p>
                )}
              </CardContent>
            </Card>
          ))}
                ...
                <Table className="w-full min-w-[400px]">
                  <TableHeader>
                    <TableRow className="bg-muted/40 hover:bg-muted/40 border-b border-border/40">
                      ...
                      {selected.map((_, i) => (
                        <TableHead key={i} className="py-4 px-6 font-semibold text-foreground text-center">
                          <span className="font-mono text-sm bg-muted text-muted-foreground px-2 py-1 rounded border border-border">Result {i + 1}</span>
                        </TableHead>
                      ))}
                    </TableRow>
                  </TableHeader>
                        ...
                        >
                          <TableCell className="py-4 px-6 font-medium text-foreground text-left">
                            <div className="flex items-center gap-1.5">
                              <span>{f.label}</span>
                              {isDifferent && (
                                <span className="text-[10px] bg-amber-500/20 text-amber-700 dark:text-amber-400 px-1.5 py-0.5 rounded font-semibold">differs</span>
                              )}
                            </div>
                          </TableCell>
                          {selected.map((item, i) => {
                            ...
                            let display: any = raw;
                            if (f.key === 'alternatives') {
                              display = raw?.map((a: any) => a.crop).join(', ') || 'None';
                            } else if (f.format) {
                              display = f.format(raw);
                            }
                            return (
                              <TableCell key={i} className="py-4 px-6 text-center">
        ...
        {selected.length > 0 && selected.length < 2 && (
          <p className="text-center text-muted-foreground mt-4">Select another saved result to compare.</p>
        )}
      </div>
    </>
  );
