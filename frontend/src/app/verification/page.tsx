...
const VerificationDashboardComponent: React.FC = () => {
  ...
  const handleBulkUpload = async (file: File) => {
    try {
      ...
      const { data } = await apiClient.post(
        "/verification/bulk/issue-credential",
        formData,
        {
      ...
      if (data.structuredErrors?.length > 0) {
        ...
        toast.error(
          `${data.structuredErrors.length} row(s) have validation errors`,
        );
      } else {
  ...
  const [rowStatuses, setRowStatuses] = useState<
    Record<
      string,
      "in_progress" | "verified" | "failed" | "linked" | "unverified" | null
    >
  >({});
  ...
  const handleVerificationSocketUpdate = useCallback(
    (data: any) => {
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <div className="flex items-center gap-1.5 text-xs text-muted-foreground mt-1">
              <span
                className={`h-2.5 w-2.5 rounded-full ${socketConnected ? "bg-emerald-500 animate-pulse" : "bg-amber-500"}`}
              />
              {socketConnected
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Upload a CSV file to issue credentials in bulk
              </p>
            </div>
            ...
            <div className="mt-6 space-y-3">
              <div className="flex items-center justify-between">
                <h4 className="text-sm font-semibold text-rose-500">
                  Validation Errors ({uploadErrors.length} row
                ...
                <Table>
                  ...
                  <TableBody>
                    {uploadErrors.map((err, i) => (
                      <TableRow
                        key={i}
                        className="border-b border-border/40 text-left"
                      >
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-muted/40 hover:bg-muted/40 border-b border-border/40 text-left">
                      ...
                      <TableHead className="py-4 px-6 font-semibold text-foreground text-right">
                        Actions
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                      ...
                      >
                        ...
                        <TableCell className="py-4 px-6">
                          {rowStatuses[item._id] === "in_progress" ? (
                            <Badge
                              variant="outline"
                              className="bg-amber-500/10 text-amber-500 border-amber-500/20 font-semibold animate-pulse"
                            >
                              In Progress
                            </Badge>
                        ...
                        <TableCell className="py-4 px-6 text-right">
                          <Button
                            onClick={() =>
                              handleVerify(item._id, item.walletAddress || "")
                            }
                            disabled={
                              !item.walletAddress ||
                              processingUserId === item._id ||
                              rowStatuses[item._id] === "in_progress"
                            }
                            size="sm"
                            className="font-medium"
