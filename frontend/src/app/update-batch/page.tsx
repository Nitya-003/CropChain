...
const UpdateBatch: React.FC = () => {
  ...
  const { canUpdateToStage, getNextAllowedStage, getRoleDisplayName } =
    useRbac();
  ...
  const nextAllowedStage = batch
    ? getNextAllowedStage(batch.currentStage)
    : null;
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Batch not found. Please check the ID and try again.";
  ...
  const handleUpdate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!canUpdateToStage(updateData.stage)) {
      toast.error(
        `You are not authorized to update to stage: ${updateData.stage}`,
      );
      return;
    }
    ...
    try {
      ...
      if (hasMetaMask()) {
        try {
          ...
          if (signer) {
            ...
            if (contract) {
              toast.loading("Minting supply chain update on-chain...", {
                id: "web3-update",
              });
              ...
              toast.success("Successfully updated on-chain!", {
                id: "web3-update",
              });
            }
          }
          ...
          toast.error(
            `Web3 update failed, queueing for background sync: ${web3Err.message || web3Err}`,
            { id: "web3-update" },
          );
        }
      }
      ...
      const updatedBatch = await realCropBatchService.updateBatch(
        batch.batchId,
        {
  ...
  const handleRequestIoTVerification = async () => {
    ...
    if (!canRequestIoT) {
      toast.error(
        "Only Transporters and Mandi operators can request IoT verification",
      );
      return;
    }
    ...
    try {
      ...
      if (ethereum) {
        ...
        toast.success(
          `IoT verification requested! Transaction: ${receipt.transactionHash}`,
        );

        // Refresh batch data to show updated status
        setTimeout(async () => {
          try {
            const updatedBatch = await realCropBatchService.getBatch(
              batch.batchId,
            );
            setBatch(updatedBatch);
          } catch (error) {
  ...
  const getTimelineEvents = (batchData: any) => {
    ...
    return batchData.updates.map((update: any) => ({
      ...
      description:
        update.notes ||
        `${t("updateBatch.processedBy", "Processed by")} ${update.actor}`,
    }));
  };
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
        ...
        <div className="flex gap-4">
          ...
          >
            ...
            <span>
              {isSearching ? t("updateBatch.searching") : t("filters.search")}
            </span>
          </button>
        </div>
      </div>
        ...
        <>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
            ...
            <Timeline
              ...
              currentStep={getStageIndex(batch.currentStage)}
            />
          </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
              ...
              <div className="space-y-4">
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  To hand custody of this batch over to the next actor,
                  authorize their wallet address below.
                </p>
                <div className="flex gap-4">
                  <input
                    type="text"
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      const loadingToast = toast.loading(
                        "Authorizing next custodian on-chain...",
                      );
                      try {
                        const contract = await getContract();
                        if (contract) {
                          ...
                          await tx.wait();
                          toast.success(
                            "Successfully authorized next custodian!",
                            { id: loadingToast },
                          );
                        ...
                        toast.error(`Handover failed: ${err.message || err}`, {
                          id: loadingToast,
                        });
                      }
                    }}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-sm text-blue-700 dark:text-blue-300 mt-1">
                      {t("updateBatch.nextStageDescription", {
                        stage: nextAllowedStage,
                      })}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-sm text-red-700 dark:text-red-300 mt-1">
                      {t("updateBatch.roleNotAuthorized", {
            ...
            <form onSubmit={handleUpdate} className="space-y-6">
              ...
              {batch &&
                (getRoleDisplayName() === "Transporter" ||
                  getRoleDisplayName() === "Market") && (
                      ...
                      >
                        {isRequestingIoT ? (
                          ...
                          <>
                            ...
                            <span>
                              {t("updateBatch.requestIoTVerification")}
                            </span>
                          </>
                        )}
                      </button>
