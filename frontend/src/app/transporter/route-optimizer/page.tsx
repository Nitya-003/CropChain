...
import { 
  ArrowLeft, MapPin, Truck, RefreshCw, Navigation, Play, Pause, ChevronRight, CheckCircle2, Award, 
  Map, ShieldAlert, GripVertical, ExternalLink 
} from 'lucide-react';
import ProtectedRoute from '../../../components/ProtectedRoute';
import { useAuth } from '../../../context/AuthContext';
import { realCropBatchService, BatchData } from '../../../services/realCropBatchService';
import { logisticsService, Waypoint, OptimizeRouteResponse } from '../../../services/logisticsService';
...
function RouteOptimizerComponent() {
      ...
      try {
        ...
        selectedBatchIds.forEach(id => {
          ...
          if (batch) {
            ...
            coords.push({
              lat: batch.currentStage === 'mandi' ? 12.9 + (Math.random() - 0.5) * 0.4 : 12.92, // mock offset around Blr
              lng: batch.currentStage === 'mandi' ? 77.5 + (Math.random() - 0.5) * 0.4 : 77.61,
  ...
  const handleWaypointComplete = () => {
    ...
    toast.success(`Arrived at Stop ${currentWaypointIndex}: ${currentWaypoint.address}`);

    if (currentWaypointIndex < routeData.orderedCoordinates.length - 1) {
      setCurrentWaypointIndex(prev => prev + 1);
    } else {
      setIsDeliveryActive(false);
      toast.success('All deliveries completed successfully! 🏆', { duration: 6000 });
    }
  };
    ...
    <div className="max-w-7xl mx-auto space-y-8 py-4">
      ...
      <div className="flex items-center justify-between border-b border-border/40 pb-6 text-left">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-foreground">Multi-Stop Route Optimizer</h1>
            <p className="text-sm text-muted-foreground">
              Calculate the most efficient path for pickup and deliveries
            </p>
          </div>
        </div>
        <Button variant="outline" size="sm" onClick={loadBatches} className="gap-1.5" disabled={isLoadingBatches}>
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card className="border-border">
            <CardHeader>
              ...
              <CardDescription>Specify the route beginning point</CardDescription>
            </CardHeader>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <input 
                    ...
                    onChange={(e) => setStartPoint(prev => ({ ...prev, address: e.target.value }))}
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input 
                    ...
                    onChange={(e) => setStartPoint(prev => ({ ...prev, lat: parseFloat(e.target.value) || 0 }))}
                  />
                </div>
                <div className="space-y-1">
                  ...
                  <input 
                    ...
                    onChange={(e) => setStartPoint(prev => ({ ...prev, lng: parseFloat(e.target.value) || 0 }))}
                  />
                </div>
              </div>
            </CardContent>
          ...
          <Card className="border-border">
            <CardHeader>
              ...
              <CardDescription>Select batches to include in optimization</CardDescription>
            </CardHeader>
                        ...
                        <div className="flex-1 min-w-0 text-left">
                          ...
                          <div className="flex items-center gap-1.5 mt-1">
                            <Badge variant="secondary" className="text-[10px] py-0 px-1.5">
                              {batch.currentStage === 'mandi' ? 'Ready Pickup' : 'In Transit'}
                            </Badge>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <Card className="border-border">
                <CardContent className="pt-6">
                  <p className="text-xs font-semibold text-muted-foreground uppercase">Total Distance</p>
              ...
              <Card className="border-border">
                <CardContent className="pt-6">
                  <p className="text-xs font-semibold text-muted-foreground uppercase">Estimated Duration</p>
                  <p className="text-2xl font-bold tracking-tight text-foreground mt-1">
              ...
              <Card className="border-border">
                <CardContent className="pt-6">
                  <p className="text-xs font-semibold text-muted-foreground uppercase">Total Stops</p>
                  <p className="text-2xl font-bold tracking-tight text-foreground mt-1">
            ...
            <Card className="border-2 border-emerald-500/30 bg-emerald-500/5 shadow-md">
              <CardContent className="p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div className="flex gap-3">
                  ...
                  <div className="text-left">
                    <div className="flex items-center gap-2">
                      ...
                      {routeData.orderedCoordinates[currentWaypointIndex].type && (
                    ...
                    <p className="text-base font-semibold text-foreground mt-0.5">
                      {routeData.orderedCoordinates[currentWaypointIndex].address}
                    </p>
                    {routeData.orderedCoordinates[currentWaypointIndex].batchId && (
                      <p className="text-xs text-muted-foreground mt-0.5 font-mono">
                        Batch: {routeData.orderedCoordinates[currentWaypointIndex].batchId}
                      </p>
                    )}
                  </div>
                </div>
                <div className="flex gap-2 w-full md:w-auto">
                  ...
                  >
                    <Button variant="outline" className="w-full gap-1.5 text-foreground hover:bg-muted">
                  ...
                  >
                    ...
                    {currentWaypointIndex === routeData.orderedCoordinates.length - 1 ? 'Finish Delivery' : 'Arrived / Next'}
                  </Button>
                </div>
              </CardContent>
            </Card>
            ...
            <div className="md:col-span-3 h-[450px]">
              {routeData ? (
                ...
                <div className="w-full h-full min-h-[450px] bg-muted/40 rounded-2xl flex flex-col items-center justify-center p-8 border border-border border-dashed">
                  ...
                  <p className="font-semibold text-foreground text-center">No Route Calculated</p>
                  <p className="text-sm text-muted-foreground text-center mt-1 max-w-[280px]">
                    Select active shipments and click "Calculate Route" to plan the path
                  </p>
                </div>
              )}
            </div>
                  ...
                  {!routeData ? (
                    ...
                    <div className="space-y-3">
                      {routeData.orderedCoordinates.map((coord, index) => {
                        ...
                        const isActive = isDeliveryActive && currentWaypointIndex === index;
                        const isCompleted = isDeliveryActive && index < currentWaypointIndex;
                        
                        let badgeColor = "bg-blue-600/10 text-blue-500 border-none capitalize";
                        if (coord.type === 'pickup') badgeColor = "bg-amber-600/10 text-amber-500 border-none capitalize";
                        if (coord.type === 'dropoff') badgeColor = "bg-emerald-600/10 text-emerald-500 border-none capitalize";
