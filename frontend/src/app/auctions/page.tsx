...
import {
  buildMarketplaceChips,
  MarketplaceFilterChips,
} from "../../components/marketplace/MarketplaceFilterChips";
    ...
    <Card className="border border-border bg-card hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group overflow-hidden">
      ...
      <CardHeader className="pb-3 text-left">
        <div className="flex justify-between items-start">
          <Badge
            variant="outline"
            className="text-xl px-2 py-0.5 rounded-xl border-none select-none"
          >
...
export default function AuctionsPage() {
  ...
  const [filters, setFilters] =
    useState<MarketplaceFilterState>(defaultFilterState);
  ...
  const filteredAuctions = useMemo(() => {
    ...
    list = list.filter((auction) => {
      if (filters.productCategory) {
        ...
        if (
          ...
          String(filters.productCategory).toLowerCase()
        )
          return false;
      }
    ...
    switch (filters.sortBy) {
      case "price_asc":
        sorted.sort(
          (a, b) =>
            Number(a.currentHighestBid || 0) - Number(b.currentHighestBid || 0),
        );
        break;
      case "price_desc":
        sorted.sort(
          (a, b) =>
            Number(b.currentHighestBid || 0) - Number(a.currentHighestBid || 0),
        );
        break;
      case "popular":
        sorted.sort(
          (a, b) =>
            Number((b as any).popularity ?? (b as any).totalBids ?? 0) -
            Number((a as any).popularity ?? (a as any).totalBids ?? 0),
        );
        break;
      case "latest":
      default:
        ...
        <section className="relative overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-slate-900 via-slate-800 to-amber-950/20 p-8 sm:p-10 text-white shadow-lg">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="space-y-4 text-left md:max-w-2xl">
              <Badge
                variant="outline"
                className="px-3 py-1 bg-amber-500/10 border-amber-500/30 text-amber-400 font-bold text-xs tracking-wider"
              >
                <Compass className="h-3.5 w-3.5 mr-1.5 animate-spin" /> LIVE
                BIDDING PORTAL
              </Badge>
              ...
              <p className="text-slate-300 text-sm leading-relaxed">
                Maximize profits for farmers and secure high-quality crop
                batches as buyers. Bid live, lock transaction contracts
                atomically, and watch market updates instantly.
              </p>
            </div>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center gap-4 bg-muted/20 rounded-3xl border border-dashed border-border p-8">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                There are no {activeTab !== "all" ? activeTab : ""} auctions
                available right now.
              </p>
            </div>
          </div>
