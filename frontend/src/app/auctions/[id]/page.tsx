...
import { ArrowLeft, Clock, Coins, User, TrendingUp, MapPin, Package, ShieldCheck, FileText } from 'lucide-react';
...
export default function AuctionRoomPage() {
  ...
  const [timeState, setTimeState] = useState<'normal' | 'warning' | 'critical' | 'ended'>('normal');

  const bidsEndRef = useRef<HTMLDivElement>(null);

  // Hook up WebSocket
  const { liveAuction, liveBids, placeNewBid, isConnected } = useAuctionSocket(auctionId);
  ...
  useEffect(() => {
    if (liveBids.length > 0) {
      ...
      setBidsList((prev) => {
        if (prev.some((b) => b._id === newLiveBid._id || (b.bidAmount === newLiveBid.bidAmount && b.userId === newLiveBid.userId))) {
          return prev;
        }
  ...
  const handlePlaceBid = (amount: number) => {
    ...
    if (amount <= auction.currentHighestBid) {
      toast.error(`Your bid must be higher than ${auction.currentHighestBid} credits.`);
      return;
    }
  ...
  if (loading) {
    return (
      <div className="max-w-7xl mx-auto py-12 flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-muted-foreground font-semibold">Connecting to Auction Room...</p>
      </div>
    );
  }
    ...
    <ProtectedRoute>
      <div className="max-w-7xl mx-auto space-y-6 py-4">
        ...
        <div className="flex justify-between items-center">
          ...
          <Badge variant="outline" className={`gap-1.5 px-3 py-1 font-bold text-xs ${
            isConnected 
            ...
            <Card className="border border-border bg-card shadow-sm text-left">
              <CardHeader className="pb-3 border-b border-border/40 bg-muted/20">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <CardTitle className="text-lg font-bold capitalize">{auction.batchDetails?.cropType} Batch</CardTitle>
                    <p className="text-xs text-muted-foreground font-mono mt-0.5">{auction.batchId}</p>
                  </div>
                </div>
              </CardHeader>
              ...
              <CardContent className="py-6 space-y-5">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <p className="text-[11px] uppercase font-bold text-muted-foreground tracking-wider flex items-center gap-1">
                      <Coins className="h-3.5 w-3.5 text-muted-foreground" /> Initial Price
                    </p>
                    <p className="text-base font-bold font-mono text-foreground">{auction.startPrice?.toLocaleString()} cr</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-[11px] uppercase font-bold text-muted-foreground tracking-wider flex items-center gap-1">
                      <Package className="h-3.5 w-3.5 text-muted-foreground" /> Crop Quantity
                    </p>
                    <p className="text-base font-bold text-foreground">{auction.batchDetails?.quantity?.toLocaleString()} kg</p>
                  </div>
                </div>

                <div className="space-y-1.5 border-t border-border/40 pt-4">
                  <p className="text-[11px] uppercase font-bold text-muted-foreground tracking-wider flex items-center gap-1">
                    <MapPin className="h-3.5 w-3.5 text-muted-foreground" /> Harvest Origin
                  </p>
                  <p className="text-sm font-semibold text-foreground">{auction.batchDetails?.origin || 'Unknown'}</p>
                </div>

                {auction.batchDetails?.certifications && (
                  <div className="space-y-1.5 border-t border-border/40 pt-4">
                    <p className="text-[11px] uppercase font-bold text-muted-foreground tracking-wider flex items-center gap-1">
                      <ShieldCheck className="h-3.5 w-3.5 text-emerald-500" /> Certifications
                    </p>
                ...
                {auction.batchDetails?.description && (
                  <div className="space-y-1.5 border-t border-border/40 pt-4">
                    <p className="text-[11px] uppercase font-bold text-muted-foreground tracking-wider flex items-center gap-1">
                      <FileText className="h-3.5 w-3.5 text-muted-foreground" /> Batch Description
                    </p>
                ...
                <div className="space-y-1.5 border-t border-border/40 pt-4">
                  <p className="text-[11px] uppercase font-bold text-muted-foreground tracking-wider flex items-center gap-1">
                    <User className="h-3.5 w-3.5 text-muted-foreground" /> Seller (Farmer)
                  </p>
                  <p className="text-sm font-semibold text-foreground">{auction.farmerName}</p>
                </div>
              </CardContent>
            </Card>
          </div>
            ...
            <Card className="border border-border bg-card shadow-sm text-left">
              <CardContent className="p-6 grid sm:grid-cols-2 gap-6 items-center">
                ...
                <div className="flex flex-col items-center sm:items-start text-center sm:text-left space-y-2 border-b sm:border-b-0 sm:border-r border-border/40 pb-6 sm:pb-0 sm:pr-6">
                  ...
                  <div className={`text-4xl font-black font-mono tracking-tight transition-all duration-300 ${
                    timeState === 'critical' ? 'text-rose-500 animate-pulse scale-105' :
                    timeState === 'warning' ? 'text-amber-500' :
                    timeState === 'ended' ? 'text-slate-400 dark:text-slate-600' :
                    'text-emerald-500'
                  }`}>
                    {timeLeft}
                  </div>
                  
                  <Badge variant="outline" className={`font-bold uppercase tracking-wider text-[10px] border ${
                    timeState === 'critical' ? 'bg-rose-500/10 text-rose-500 border-rose-500/30' :
                    timeState === 'warning' ? 'bg-amber-500/10 text-amber-500 border-amber-500/30' :
                    timeState === 'ended' ? 'bg-slate-100 text-slate-500 border-slate-300/30 dark:bg-slate-800/40' :
                    'bg-emerald-500/10 text-emerald-500 border-emerald-500/30'
                  }`}>
                    {timeState === 'critical' ? 'HURRY! Closing Soon' :
                     timeState === 'warning' ? 'Closing Soon' :
                     timeState === 'ended' ? 'Completed' :
                     'Bidding Active'}
                  </Badge>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-end">
                    <div>
                      <p className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">Current High Bid</p>
                    ...
                    {auction.highestBidderName && (
                      <div className="text-right">
                        <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Top Bidder</p>
                        <Badge variant="outline" className="mt-1 font-semibold border-primary/20 bg-primary/5 text-primary text-[11px]">
                          {auction.highestBidderName}
                        </Badge>
                      </div>
                    )}
                  ...
                  {timeState !== 'ended' ? (
                    isFarmer ? (
                      <div className="bg-muted p-4 rounded-xl border border-border text-center text-xs text-muted-foreground italic">
                        You created this auction. Farmers cannot bid on their own crops.
                      </div>
                    ) : (
                    ...
                    <div className="bg-muted p-4 rounded-xl border border-border text-center">
                      <p className="text-xs font-bold text-foreground">Auction Complete</p>
            ...
            <Card className="border border-border bg-card shadow-sm text-left flex flex-col h-[400px] overflow-hidden">
              <CardHeader className="pb-3 border-b border-border/40 flex flex-row items-center justify-between shrink-0 bg-muted/10">
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-sm font-bold uppercase tracking-wider">Live Bid Stream</CardTitle>
                </div>
                <span className="text-[10px] font-bold text-muted-foreground uppercase">{bidsList.length} total bids</span>
              </CardHeader>
              ...
              <CardContent className="flex-1 overflow-y-auto p-4 space-y-3.5">
                {bidsList.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center text-xs text-muted-foreground italic space-y-2 py-20">
                    ...
                    <span>No bids placed yet. Be the first to place a bid!</span>
                  </div>
                ) : (
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            {/* User Initial Circle */}
                            <div className={`h-8 w-8 rounded-full flex items-center justify-center text-xs font-black border ${
                              isHighestBid 
                          ...
                          <div className="text-right">
                            <p className={`text-base font-black font-mono tracking-tight ${
                              isHighestBid ? 'text-amber-600 dark:text-amber-400' : 'text-foreground/80'
                            }`}>
                              {bid.bidAmount?.toLocaleString()}
                              <span className="text-[10px] text-muted-foreground font-normal font-sans ml-0.5">cr</span>
                            </p>
                          </div>
                        </div>
                      );
