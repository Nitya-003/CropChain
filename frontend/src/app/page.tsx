...
const Home: React.FC = () => {
  ...
  const stages = [
    {
      icon: Sprout,
      title: t("home.stages.farmer.title"),
      description: t("home.stages.farmer.description"),
      color: "text-emerald-500 bg-emerald-500/10",
    },
    {
      icon: Building2,
      title: t("home.stages.mandi.title"),
      description: t("home.stages.mandi.description"),
      color: "text-blue-500 bg-blue-500/10",
    },
    {
      icon: Truck,
      title: t("home.stages.transport.title"),
      description: t("home.stages.transport.description"),
      color: "text-amber-500 bg-amber-500/10",
    },
    {
      icon: Store,
      title: t("home.stages.retailer.title"),
      description: t("home.stages.retailer.description"),
      ...
      <section className="relative overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-slate-50 via-white to-emerald-50/30 dark:from-slate-950 dark:via-slate-900 dark:to-emerald-950/20 py-20 px-6 sm:px-12 shadow-sm">
        ...
        <div className="absolute top-6 right-6">
          {isConnected ? (
            <Badge
              variant="outline"
              className="flex items-center gap-1.5 border-emerald-500/30 bg-emerald-500/5 px-3 py-1 text-xs font-semibold text-emerald-600 dark:text-emerald-400"
            >
            ...
            <Badge
              variant="outline"
              className="flex items-center gap-1.5 border-amber-500/30 bg-amber-500/5 px-3 py-1 text-xs font-semibold text-amber-600 dark:text-amber-400"
            >
              <span className="relative flex h-2 w-2">
        ...
        <div className="max-w-4xl mx-auto text-center space-y-6">
          <Badge
            variant="outline"
            className="px-3 py-1 text-sm bg-background/50 border-border/60"
          >
            <Activity className="h-3.5 w-3.5 mr-1.5 text-primary" /> Immutable
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4">
            <Link href="/add-batch">
              <Button
                size="lg"
                className="w-full sm:w-auto px-8 font-semibold shadow-md shadow-primary/10"
              >
                {t("home.getStarted")}
            ...
            <Link href="/track-batch">
              <Button
                size="lg"
                variant="outline"
                className="w-full sm:w-auto px-8 font-semibold bg-background/50"
              >
                {t("nav.trackBatch")}
              </Button>
            </Link>
      ...
      <section className="space-y-10">
        <div className="text-center max-w-2xl mx-auto space-y-3">
          ...
          <p className="text-muted-foreground text-sm">
            Track crop journey securely with distributed cryptographic
        ...
        <div className="grid md:grid-cols-4 gap-6 max-w-6xl mx-auto">
          {stages.map((stage, index) => (
            <Card
              key={index}
              className="relative group hover:shadow-md hover:-translate-y-1 transition-all duration-300 border-border bg-card"
            >
              {/* Connecting Line */}
              ...
              <CardHeader className="flex flex-col items-center text-center pb-2">
                <div
                  className={`p-4 rounded-full ${stage.color} mb-4 transform group-hover:scale-105 transition-transform duration-300`}
                >
      ...
      <section className="border border-border bg-card rounded-3xl p-8 sm:p-12 shadow-sm max-w-6xl mx-auto space-y-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {benefits.map((benefit, index) => (
            <div
              key={index}
              className="p-6 rounded-2xl hover:bg-emerald-500/5 dark:hover:bg-emerald-500/10 border border-transparent hover:border-emerald-500/10 transition-all duration-300 space-y-3"
            >
        ...
        <div className="relative overflow-hidden bg-gradient-to-r from-slate-900 to-emerald-950 border border-slate-800 rounded-3xl p-10 sm:p-16 text-white text-center space-y-6 shadow-xl">
          ...
          <div className="pt-4">
            <Link href="/add-batch">
              <Button
                size="lg"
                className="bg-white hover:bg-slate-100 text-slate-900 font-semibold px-8"
              >
