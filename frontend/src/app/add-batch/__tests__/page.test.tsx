...
function fillRequiredFields() {
  fireEvent.change(document.querySelector('input[name="farmerName"]')!, {
    target: { name: "farmerName", value: "John Farmer" },
  });
  fireEvent.change(document.querySelector('input[name="farmerAddress"]')!, {
    target: { name: "farmerAddress", value: "0x123" },
  });
  ...
  fireEvent.change(quantityInput, {
    target: { name: "quantity", value: "100" },
  });
  const yesterday = new Date(Date.now() - 86400000).toISOString().split("T")[0];
  const harvestDateInput = document.querySelector('input[name="harvestDate"]')!;
  fireEvent.change(harvestDateInput, {
    target: { name: "harvestDate", value: yesterday },
  });
  fireEvent.change(document.querySelector('input[name="origin"]')!, {
    target: { name: "origin", value: "Farmville" },
  });
}
...
describe("AddBatch Page", () => {
  ...
  it("shows Access Denied for non-farmer users", () => {
    ...
    expect(
      screen.getByText(/Only farmers can create batches/),
    ).toBeInTheDocument();
  });
  ...
  it("shows success view after batch creation", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/batch\.batchCreatedSuccess/),
      ).toBeInTheDocument();
