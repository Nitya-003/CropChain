  ...
  async createBatch(data: Record<string, unknown>): Promise<CropBatch> {
    ...
    if (navigator.onLine) {
      try {
        ...
        console.warn(
          "[OfflineCropBatchService] Online creation failed, falling back to offline mode:",
          error,
        );
      }
    }
      ...
      updates: [
        {
          ...
          notes: String(
            data.description || "Initial harvest recorded (offline)",
          ),
        },
      ],
      qrCode,
  ...
  async updateBatch(
    ...
    if (navigator.onLine) {
      try {
        ...
        console.warn(
          "[OfflineCropBatchService] Online update failed, falling back to offline mode:",
          error,
        );
      }
    }
    ...
    const update = {
      ...
      timestamp: String(
        (updateData as any).timestamp || new Date().toISOString(),
      ),
    ...
    try {
      ...
      console.warn(
        "[OfflineCropBatchService] Could not load existing batch, creating optimistic batch for offline update:",
        error,
      );
      batch = {
        batchId,
        ...
        harvestDate: String(
          (updateData as any).harvestDate || new Date().toISOString(),
        ),
    ...
    batch.currentStage = String(
      (updateData as any).stage || batch.currentStage,
    );
  ...
  async getBatch(batchId: string): Promise<CropBatch> {
    ...
    if (navigator.onLine) {
      try {
        ...
        console.warn(
          "[OfflineCropBatchService] Failed to fetch batch online:",
          error,
        );
      }
    }
    ...
    const pendingBatch = pendingBatches.find(
      (b) => b.data.batchId === batchId || b.id === batchId,
    );

    if (pendingBatch) {
      const qrCode = await this.generateQRCode(batchId);
      const pendingData = pendingBatch.data as Omit<
        CropBatch,
        "qrCode" | "syncStatus" | "pendingId"
      >;
      return {
        ...pendingData,
        batchId: batchId,
  ...
  async getDashboardStats(): Promise<Record<string, unknown>> {
    ...
    if (navigator.onLine) {
      try {
        ...
        console.warn(
          "[OfflineCropBatchService] Failed to fetch dashboard stats online:",
          error,
        );
      }
    }
    ...
    return {
      stats: {
        ...
        totalFarmers: new Set(pendingBatches.map((b) => b.data.farmerName))
          .size,
