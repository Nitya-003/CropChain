...
describe("SyncManager - Complete Implementation Test", () => {
  ...
  describe("Phase 3: Promise Rejection Handling & User Notifications", () => {
    test("should handle constructor errors gracefully", async () => {
      const mockGetPendingCount = (
        offlineStorage.getPendingCount as jest.Mock
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        "[SyncManager] Initial sync check failed:",
        expect.any(Error),
      );
    ...
    test("should show success notification when all data synced", async () => {
      ...
      const mockGetPendingCount = (
        offlineStorage.getPendingCount as jest.Mock
      ).mockResolvedValue({ batches: 0, updates: 0 });
      const mockSyncPendingItems = jest
        .spyOn(syncManager as any, "syncPendingItems")
    ...
    test("should handle retryFailed errors with user notification", async () => {
      const mockToast = toast as jest.MockedFunction<typeof toast>;
      const mockGetAllPendingBatches = (
        offlineStorage.getAllPendingBatches as jest.Mock
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        "[SyncManager] Failed to retry failed items:",
        expect.any(Error),
      );
    ...
    test("should handle online event sync trigger errors", async () => {
      ...
      expect(mockToast.error).toHaveBeenCalledWith(
        "Failed to start sync after connection restore.",
        {
          duration: 3000,
          position: "top-right",
        },
  ...
  describe("Edge Cases", () => {
    test("should not sync when already in progress", async () => {
      ...
      const mockSyncPendingItems = jest.spyOn(
        syncManager as any,
        "syncPendingItems",
      );
