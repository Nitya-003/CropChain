...
class CropBatchService {
    ...
    const sampleBatches = [
      {
        ...
        description:
          "High-quality Basmati rice grown using traditional methods",
      },
      {
  ...
  async getDashboardStats(): Promise<any> {
    ...
    const totalQuantity = batchesArray.reduce(
      (sum, batch) => sum + batch.quantity,
      0,
    );
    const recentBatches = batchesArray.filter((batch) => {
