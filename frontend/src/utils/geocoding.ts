...
const KEYWORD_MAP: Array<{ keywords: string[]; coords: Coordinates }> = [
  { keywords: ['bangalore', 'bengaluru', 'blr'], coords: { lat: 12.9716, lng: 77.5946 } },
  { keywords: ['mumbai', 'bombay', 'mumb'], coords: { lat: 19.0760, lng: 72.8777 } },
  { keywords: ['delhi', 'new delhi', 'ncr'], coords: { lat: 28.6139, lng: 77.2090 } },
  { keywords: ['punjab', 'amritsar', 'ludhiana'], coords: { lat: 31.1471, lng: 75.3412 } },
  { keywords: ['haryana', 'gurgaon', 'panipat'], coords: { lat: 29.0588, lng: 76.0856 } },
  { keywords: ['pune', 'maharashtra'], coords: { lat: 18.5204, lng: 73.8567 } },
  { keywords: ['chennai', 'madras'], coords: { lat: 13.0827, lng: 80.2707 } },
  { keywords: ['hyderabad', 'secunderabad'], coords: { lat: 17.3850, lng: 78.4867 } },
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data && data.length > 0) {
        ...
        memoryCache[query] = coords;
        try {
          localStorage.setItem(`geocode_cache_${query}`, JSON.stringify(coords));
        } catch (e) {
    ...
    console.warn(`Nominatim geocoding failed for "${query}", using deterministic fallback.`, error);
  }
