...
const { mockAuthService, mockUser } = vi.hoisted(() => {
  const user = { id: '1', name: 'Test Farmer', email: 'farmer@test.com', role: 'farmer' as const };
  const service = {
    login: vi.fn(),
    register: vi.fn(),
...
function TestConsumer() {
  const { user, isAuthenticated, isLoading, login, logout, connectWallet, walletLogin, isWalletConnected } = useAuth();
  return (
    <div>
      <span data-testid="loading">{isLoading ? 'loading' : 'idle'}</span>
      <span data-testid="authenticated">{isAuthenticated ? 'yes' : 'no'}</span>
      <span data-testid="wallet-connected">{isWalletConnected ? 'yes' : 'no'}</span>
      <span data-testid="user-role">{user?.role || 'none'}</span>
      <button data-testid="btn-login" onClick={() => login({ email: 'farmer@test.com', password: 'pass' })}>Login</button>
      <button data-testid="btn-logout" onClick={() => logout()}>Logout</button>
      <button data-testid="btn-connect-wallet" onClick={() => connectWallet()}>Connect Wallet</button>
      <button data-testid="btn-wallet-login" onClick={() => walletLogin()}>Wallet Login</button>
    </div>
  );
}
...
describe('AuthContext', () => {
  ...
  it('connectWallet with ethereum sets wallet as connected', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId('loading')).toHaveTextContent('idle'));
    expect(screen.getByTestId('wallet-connected')).toHaveTextContent('no');
    (window as any).ethereum = { request: vi.fn().mockResolvedValue(['0x123']), selectedAddress: '0x123' };
    act(() => { screen.getByTestId('btn-connect-wallet').click(); });
    await waitFor(() => {
  ...
  it('walletLogin with ethereum completes full flow', async () => {
    ...
    mockAuthService.walletLogin.mockResolvedValue({ user: { ...mockUser, walletAddress: '0x123' } });
    renderWithAuth();
    await waitFor(() => expect(screen.getByTestId('loading')).toHaveTextContent('idle'));
    (window as any).ethereum = { request: mockRequest, selectedAddress: '0x123' };
    act(() => { screen.getByTestId('btn-wallet-login').click(); });
    await waitFor(() => {
      expect(screen.getByTestId('authenticated')).toHaveTextContent('yes');
    });
