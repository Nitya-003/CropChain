...
describe('BatchSyncBadge', () => {
  ...
  it('renders correct tooltip for synced state', () => {
    ...
    expect(screen.getByTitle('Successfully synced to the blockchain')).toBeInTheDocument();
  });

  it('renders correct tooltip for failed state', () => {
    render(<BatchSyncBadge syncStatus="failed" />);
    expect(screen.getByTitle('Failed to sync. Will retry automatically when online')).toBeInTheDocument();
  });
});
