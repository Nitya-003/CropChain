...
export const JourneyStageNode: React.FC<JourneyStageNodeProps> = ({
  ...
  const getStageColorClass = () => {
    if (!isCompleted && !isCurrent)
      return "border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-400";
  ...
  const getStageName = () => {
    switch (update.stage) {
      ...
      case "mandi":
        return t("status.atMandi", "At Mandi (Quality Check)");
      case "transport":
        return t("status.inTransit", "In Transit (Cold Chain)");
      case "retailer":
        return t("status.atRetailer", "At Retailer");
      default:
        return update.stage;
    }
  };
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div className="flex items-center gap-3">
          <div
            className={`flex h-10 w-10 items-center justify-center rounded-full border-2 ${getStageColorClass()} ${
          ...
          <div>
            <h3
              className={`font-bold text-lg capitalize ${getStageHeaderColor()}`}
            >
              {getStageName()}
            </h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4 text-sm text-gray-600 dark:text-gray-300">
        <div className="flex items-center gap-2">
          ...
          <span className="truncate">
            <strong>{t("journey.updated_by", "Actor")}:</strong> {update.actor}
          </span>
        </div>
        <div className="flex items-center gap-2">
          <MapPin className="h-4 w-4 text-gray-400 flex-shrink-0" />
          <span className="truncate">
            <strong>{t("batch.location", "Location")}:</strong>{" "}
            {update.location}
          </span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <span className="truncate">
            <strong>{t("batch.lastUpdated", "Date")}:</strong>{" "}
        ...
        <div className="border-t border-gray-100 dark:border-gray-800 pt-4 mb-4">
          ...
          {isSpoiled && (
            <div className="mb-3 flex items-center gap-2 bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900 rounded-xl p-3 text-red-700 dark:text-red-400">
              ...
              <span className="text-xs font-semibold">
                WARNING: Cold Chain Breached. Crop quality compromised due to
          ...
          <div className="flex gap-3 flex-wrap">
            {temperature !== undefined && (
              <span
                className={`telemetry-badge telemetry-badge-${getTempStatus(temperature)}`}
              >
            ...
            {humidity !== undefined && (
              <span
                className={`telemetry-badge telemetry-badge-${getHumidityStatus(humidity)}`}
              >
      ...
      {txHash && (
        <div className="flex items-center justify-between border-t border-gray-100 dark:border-gray-800 pt-4 flex-wrap gap-2 text-xs">
          <div className="flex items-center gap-1.5 text-green-600 dark:text-green-400 font-semibold bg-green-50 dark:bg-green-950/20 px-2.5 py-1 rounded-full">
            ...
            <span>
              {t("journey.verified_on_chain", "Verified on Blockchain")}
            </span>
          </div>
          <a
