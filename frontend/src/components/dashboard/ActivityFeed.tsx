      ...
      <CardHeader className="flex flex-row items-center justify-between border-b pb-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Personalized activity feed for role:{" "}
      ...
      <div className="p-4 bg-muted/20 border-b flex flex-wrap items-end gap-3">
        ...
        <div className="min-w-[150px]">
          ...
          >
            ...
            <option value="shipment_status_updated">
      ...
      <CardContent className="p-0">
        {loading ? (
          ...
          <div className="p-12 text-center text-muted-foreground">
            ...
            <p className="text-xs">
              Adjust your search filters or check back later for updates.
            </p>
          </div>
        ) : (
          <div className="divide-y divide-border/60">
            {activities.map((item, index) => {
              const { icon: EventIcon, colorClass } = getEventStyles(
                item.eventType,
              );
              return (
                <div
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground font-medium">
                      {item.batchId && (
                        <div className="flex items-center gap-1.5">
                          <span className="font-semibold text-primary bg-primary/5 px-2 py-0.5 rounded border border-primary/10">
                            Batch: {item.batchId}
                          </span>
                        </div>
                      )}
