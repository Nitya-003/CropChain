  ...
  describe("GET /api/verification/unverified", () => {
    test("should apply search, role, and date range filters correctly", async () => {
      ...
      const response = await request(app)
        .get(
          "/api/verification/unverified?search=John&role=farmer&fromDate=2026-06-01&toDate=2026-06-03&sortBy=name&sortOrder=asc&page=1&limit=5",
        )
        .expect(200);
  ...
  describe("CSV Exports", () => {
    test("should stream unverified users CSV export and format fields correctly", async () => {
      ...
      expect(csvLines[1]).toContain(
        "John Farmer,john@farmer.com,farmer,0xabc1230000000000000000000000000000000001",
      );
      expect(csvLines[2]).toContain(
        "Bob Farmer,bob@farmer.com,farmer,0xabc1230000000000000000000000000000000003",
      );
    });
    ...
    test("should stream verified users CSV export including verified by email", async () => {
      ...
      expect(csvLines[0]).toBe(
        "Name,Email,Role,Wallet Address,Verified At,Verified By Email",
      );
      expect(csvLines[1]).toContain(
        "Alice Mandi,alice@mandi.com,mandi_officer,0xabc1230000000000000000000000000000000002,2026-06-03T12:00:00.000Z,verifier@admin.com",
      );
    });
  });
});
