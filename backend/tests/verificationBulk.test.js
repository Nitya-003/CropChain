...
const mockUser = {
  findById: jest.fn((id) =>
    Promise.resolve(
      mockUsersDb.find((u) => u._id === id || u._id.toString() === id),
    ),
  ),
  findOne: jest.fn(({ email }) =>
    Promise.resolve(mockUsersDb.find((u) => u.email === email)),
  ),
};
...
const mockSecurityService = {
  ...
  createFingerprint: jest.fn(({ action, actorId, userId, walletAddress }) => {
    return crypto
      .createHash("sha256")
      .update(`${action}:${actorId}:${userId}:${walletAddress}`)
      .digest("hex");
  }),
...
describe("Bulk Verification Jobs via CSV Upload", () => {
  beforeEach(() => {
    ...
    mockSecurityService.reserveIdempotencyKey.mockResolvedValue({
      reserved: true,
    });
  });
  ...
  describe("CSV Parser Helper Unit Tests", () => {
    test("should parse standard CSV with headers correctly", () => {
      ...
      expect(records[0].walletaddress).toBe(
  ...
  describe("Job Processing Logic (bulkVerificationService.processJob)", () => {
    test("should successfully initiate challenges for rows without signatures", async () => {
      const job = await mockBulkJob.create({
        totalRows: 1,
        actorId: "admin-id",
      });
      const records = [
      ...
      expect(updatedJob.results[0].details.challengeId).toBe(
        "mock-challenge-uuid",
      );
    ...
    test("should skip duplicate row processing due to idempotency checks", async () => {
      ...
      mockSecurityService.reserveIdempotencyKey.mockImplementation(() => {
        ...
        if (reserveCount > 1) {
          return Promise.resolve({
            ...
            record: {
              state: "completed",
              response: { success: true, message: "Replayed response" },
            },
          });
        }
      ...
      expect(updatedJob.results[1].details.message).toContain(
        "already processed",
      );
    });
  });
