...
jest.mock("../utils/verificationControllerHelpers", () => ({
  handleZodValidation: jest
    .fn()
    .mockReturnValue({
      ok: true,
      data: { userId: "target-user", reason: "test-reason" },
    }),
...
describe("verification controller replay protection - revokeCredential", () => {
  ...
  test("requires Idempotency-Key header and calls idempotency helper", async () => {
    ...
    didService.revokeCredential.mockResolvedValue({
      success: true,
    ...
    expect(didService.revokeCredential).toHaveBeenCalledWith(
      "target-user",
      "admin-1",
      "test-reason",
    );
