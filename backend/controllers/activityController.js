...
exports.getActivities = async (req, res) => {
    try {
        if (!isAdminRole(req.user.role)) {
            return res.status(403).json(apiResponse.forbiddenResponse('Admin access required'));
        }

        const { role, eventType, batchId, startDate, endDate, page = 1, limit = 10 } = req.query;
        const query = {};
        ...
        res.status(500).json(apiResponse.errorResponse('Failed to fetch activities', 'ACTIVITY_FETCH_ERROR', 500));
    }
};
...
exports.getActivityFeed = async (req, res) => {
    try {
        ...
        const { eventType, batchId, startDate, endDate, page = 1, limit = 10 } = req.query;
        ...
        res.status(500).json(apiResponse.errorResponse('Failed to fetch activity feed', 'FEED_FETCH_ERROR', 500));
    }
};
...
exports.getActivityById = async (req, res) => {
    try {
        ...
        if (!activity) {
            return res.status(404).json(apiResponse.notFoundResponse('Activity', req.params.id));
        }
        ...
        if (!authorizedActivity) {
            return res.status(403).json(apiResponse.forbiddenResponse('You are not authorized to view this activity'));
        }

        res.json(apiResponse.successResponse({ activity }, 'Activity retrieved successfully'));
    } catch (error) {
        logger.error('Error fetching activity by ID', { error: error.message });
        res.status(500).json(apiResponse.errorResponse('Failed to fetch activity', 'ACTIVITY_BY_ID_ERROR', 500));
    }
};
