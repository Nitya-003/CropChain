...
const buildUserQueryFilter = (query, isVerified) => {
  ...
  const sortBy =
    query.sortBy || (isVerified ? "verification.verifiedAt" : "createdAt");
...
const withVerificationAction = async ({
  ...
  try {
    ...
    const { walletAddress, signature, nonce, expiresAt, userId } =
      validatedData;
...
const generateLinkWalletChallenge = async (req, res) => {
  try {
    const validatedData = validateOrRespond(
      res,
      linkWalletChallengeSchema,
      req.body,
    );
    ...
    res.json(
      apiResponse.successResponse(
        { challenge },
        "Wallet linking challenge generated",
      ),
    );
  } catch (error) {
...
const generateIssueCredentialChallenge = async (req, res) => {
  try {
    const validatedData = validateOrRespond(
      res,
      issueCredentialChallengeSchema,
      req.body,
    );
    ...
    res.json(
      apiResponse.successResponse(
        { challenge },
        "Credential issuance challenge generated",
      ),
    );
  } catch (error) {
    return handleServerError(res, error, {
...
const revokeCredential = async (req, res) => {
  try {
    ...
    return await handleRevokeIdempotency({
      ...
      idempotencyKey,
      execute: async () => {
        const result = await didService.revokeCredential(
          userId,
          adminId,
          reason,
        );
...
const getVerifiedUsers = async (req, res) => {
  try {
    ...
    const users = await User.find(filter)
      .select(
        "name email role walletAddress verification.verifiedAt verification.verifiedBy",
      )
...
const exportUnverifiedUsers = async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="unverified_users.csv"',
    );
...
const exportVerifiedUsers = async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="verified_users.csv"',
    );
        ...
        [
          ...
          escapeCSV(
            user.verification?.verifiedAt
              ? user.verification.verifiedAt.toISOString()
              : "",
          ),
...
const getVerificationEvents = async (req, res) => {
  try {
    const validatedQuery = validateOrRespond(
      res,
      getVerificationEventsSchema,
      req.query,
    );
    if (!validatedQuery) {
      return;
...
const bulkIssueCredentials = async (req, res) => {
  try {
    if (!req.file) {
      return res
        .status(400)
        .json(apiResponse.validationErrorResponse(["CSV file is required"]));
    }

    const csvText = req.file.buffer.toString("utf8");

    // Defense-in-depth: limit parsed input size (in bytes) independent of multer.
    const MAX_BULK_CSV_TEXT_BYTES =
      parseInt(process.env.MAX_BULK_CSV_TEXT_BYTES, 10) || 5 * 1024 * 1024;
    if (Buffer.byteLength(csvText, "utf8") > MAX_BULK_CSV_TEXT_BYTES) {
      return res
        .status(400)
        .json(
          apiResponse.validationErrorResponse([
            `CSV file too large. Max allowed is ${MAX_BULK_CSV_TEXT_BYTES} bytes`,
          ]),
        );
    }
    ...
    if (!Array.isArray(recordsRaw) || recordsRaw.length === 0) {
      return res
        .status(400)
        .json(
          apiResponse.validationErrorResponse(["CSV file is empty or invalid"]),
        );
    }
    ...
    if (!headerValidation.ok) {
      return res
        .status(400)
        .json(apiResponse.validationErrorResponse([headerValidation.error]));
    }
    ...
    if (normalizedRecords.length === 0) {
      return res
        .status(400)
        .json(apiResponse.validationErrorResponse(["CSV has no valid rows"]));
    }
...
const getBulkJobStatus = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(jobId)) {
      return res
        .status(400)
        .json(apiResponse.validationErrorResponse(["Invalid job ID format"]));
    }
    ...
    if (!job) {
      return res
        .status(404)
        .json(
          apiResponse.errorResponse("Bulk verification job not found", 404),
        );
    }
    ...
    res.json(
      apiResponse.successResponse(
        job,
        "Bulk verification job retrieved successfully",
      ),
    );
  } catch (error) {
    return handleServerError(res, error, {
...
const streamBulkJobEvents = async (req, res) => {
  ...
  const POLL_INTERVAL_MS =
    parseInt(process.env.BULK_JOB_SSE_POLL_INTERVAL_MS, 10) || 1000;
  const HEARTBEAT_INTERVAL_MS =
    parseInt(process.env.BULK_JOB_SSE_HEARTBEAT_INTERVAL_MS, 10) || 15000;
...
const retryBulkFailedRows = async (req, res) => {
  try {
      ...
      return res
        ...
        .json(
          apiResponse.errorResponse(
            "Bulk verification job must be completed before retry",
            400,
          ),
        );
    }
    ...
    const adminId = req.user.id;
    const failedRows = Array.isArray(job.results)
      ? job.results.filter((r) => r?.status === "failure")
      : [];
    ...
    if (!failedRows.length) {
      return res
        .status(400)
        .json(
          apiResponse.errorResponse("No failed rows found for this job", 400),
        );
    }

    const retryJob = await bulkVerificationService.retryJob(
      jobId,
      failedRows,
      adminId,
    );
    if (!retryJob) {
      return res
        ...
        .json(
          apiResponse.errorResponse(
            "Retry could not be created (missing originalInput)",
            400,
          ),
        );
    }
