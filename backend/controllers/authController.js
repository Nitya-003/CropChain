...
const loginUser = async (req, res) => {
  try {
    ...
    if (user && (await bcrypt.compare(password, user.password))) {
      ...
      return res
        .status(401)
        .json(apiResponse.unauthorizedResponse("Invalid email or password"));
    }
  } catch (error) {
    return res
      .status(500)
      .json(apiResponse.errorResponse("Login failed", "LOGIN_FAILED", 500));
  }
};
...
const walletLogin = async (req, res) => {
  try {
    ...
    logger.error("Wallet login error", {
      error: error.message,
      stack: error.stack,
    });
    return res
...
const walletRegisterSchema = z.object({
  ...
  role: z
    .enum(["farmer", "transporter"], {
      errorMap: () => ({
        message:
          "Invalid role. Only farmer and transporter accounts can be created publicly.",
      }),
    })
    .default(ROLES.FARMER),
});
...
const walletRegister = async (req, res) => {
  try {
    ...
    const existingUser = await User.findOne({
      $or: [{ email }, { walletAddress: normalizedAddress }],
    });
    ...
    if (existingUser) {
      return res
        ...
        .json(
          apiResponse.conflictResponse(
            "User already exists with this email or wallet",
          ),
        );
    }
    ...
    logger.error("Wallet registration error", {
      error: error.message,
      stack: error.stack,
    });
    if (error.code === 11000) {
      return res
        .status(409)
        .json(apiResponse.conflictResponse("User already exists"));
    }
    return res
      .status(500)
      .json(
...
const refreshSession = async (req, res) => {
  try {
    ...
    if (!refreshToken) {
      return res
        .status(401)
        .json(apiResponse.unauthorizedResponse("Refresh token is required"));
    }
