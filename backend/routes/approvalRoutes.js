...
router.post(
  ...
  requirePermissions(
    ...
    PERMISSIONS.BATCH_CONTAMINATED,
    PERMISSIONS.BATCH_DESTROY,
  ),
  async (req, res) => {
    try {
      ...
      logger.info(
        `[APPROVAL] Created request ${result.requestId} by ${req.user.email}`,
      );
...
router.get(
  "/",
  protect,
  requirePermissions(PERMISSIONS.BATCH_VIEW_ALL),
  async (req, res) => {
    try {
...
router.get("/pending", protect, inspectorOnly, async (req, res) => {
  try {
    const requests = await MultisigService.getRequestsNeedingSignature(
      req.user._id,
    );
    res.json({ success: true, count: requests.length, data: requests });
  } catch (error) {
...
router.get("/:requestId", protect, async (req, res) => {
  try {
    const approval = await MultisigService.getRequestDetails(
      req.params.requestId,
    );
    const isInitiator =
      approval.initiatedBy._id.toString() === req.user._id.toString();
...
router.post("/:requestId/sign", protect, inspectorOnly, async (req, res) => {
  try {
    ...
    logger.info(
      `[APPROVAL] Signature added to ${req.params.requestId} by ${req.user.email}: ${value.decision}`,
    );
    ...
    res.json({
      ...
      message:
        result.status === "approved"
          ? "Action has been approved and executed"
          : `Signature recorded. ${result.approvalCount}/${result.requiredApprovals} approvals`,
      data: result,
    });
  } catch (error) {
    ...
    res
      .status(400)
      .json({ error: "Failed to add signature", message: error.message });
  }
});
...
router.post("/:requestId/cancel", protect, async (req, res) => {
  try {
    ...
    const result = await MultisigService.cancelRequest(
      req.params.requestId,
      req.user._id,
      reason || "Cancelled by user",
    );
    logger.info(
      `[APPROVAL] Cancelled ${req.params.requestId} by ${req.user.email}`,
    );
    res.json({
      success: true,
...
router.get("/batch/:batchId", protect, async (req, res) => {
  try {
    const history = await MultisigService.getBatchApprovalHistory(
      req.params.batchId,
    );
...
router.post(
  "/recall/:batchId",
  protect,
  requirePermissions(PERMISSIONS.BATCH_RECALL),
  async (req, res) => {
    try {
      ...
      res
        ...
        .json({
          success: true,
          message:
            "Recall approval request created. Waiting for inspector approvals.",
          data: result,
        });
    } catch (error) {
...
router.post(
  "/contaminate/:batchId",
  protect,
  requirePermissions(PERMISSIONS.BATCH_CONTAMINATED),
  async (req, res) => {
    try {
      const { justification, evidence } = req.body;
      if (!justification || justification.length < 20)
      ...
      res
        ...
        .json({
          success: true,
          message:
            "Contamination approval request created. Waiting for inspector approvals.",
          data: result,
        });
    } catch (error) {
      ...
      res
        .status(400)
        .json({
          error: "Failed to create contamination request",
          message: error.message,
        });
    }
  },
...
router.post(
  "/destroy/:batchId",
  protect,
  requirePermissions(PERMISSIONS.BATCH_DESTROY),
  async (req, res) => {
    try {
      const { justification, evidence } = req.body;
      if (!justification || justification.length < 20)
      ...
      res
        ...
        .json({
          success: true,
          message:
            "Destruction authorization request created. Waiting for inspector approvals.",
          data: result,
        });
    } catch (error) {
      ...
      res
        .status(400)
        .json({
          error: "Failed to create destruction request",
          message: error.message,
        });
    }
  },
...
router.post(
  "/quality-check/:batchId",
  protect,
  requirePermissions(PERMISSIONS.BATCH_QUALITY_CHECK),
  async (req, res) => {
    try {
      const { justification, evidence } = req.body;
      if (!justification || justification.length < 20)
      ...
      const result = await MultisigService.createApprovalRequest({
        batchId: req.params.batchId,
        actionType: "batch_quality_check",
        initiatedBy: req.user._id,
        justification,
        evidence: evidence || [],
      });
      res
        ...
        .json({
          success: true,
          message:
            "Quality check approval request created. Waiting for inspector approvals.",
          data: result,
        });
    } catch (error) {
      ...
      res
        .status(400)
        .json({
          error: "Failed to create quality check request",
          message: error.message,
        });
    }
  },
