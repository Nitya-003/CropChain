...
const MAX_BULK_CSV_BYTES =
  parseInt(process.env.MAX_BULK_CSV_BYTES, 10) || 5 * 1024 * 1024; // 5MB
...
router.get("/bulk/template", protect, adminOnly, (req, res) => {
  ...
  res.setHeader(
    "Content-Disposition",
    "attachment; filename=bulk-verification-template.csv",
  );
  res.send(csvContent);
});
