...
router.post('/batch-query', batchLimiter, protect, validateRequest(chatSchema), async (req, res) => {
    try {
        const { message, context } = req.body;
        logger.info('AI batch query request', { ip: req.ip, messagePreview: message.substring(0, 50), context });
        ...
        if (acceptsEventStream) {
            ...
            res.end();
            logger.info('AI batch query streamed response generated', { ip: req.ip });
            return;
        }
        ...
        logger.error('AI batch query error', { error: error.message, stack: error.stack });

        if (res.headersSent) {
            res.write(`event: error\n`);
            res.write(`data: ${JSON.stringify({
                error: "I'm sorry, I'm having trouble processing your query right now. Please try asking about specific supply chain batches or transit times."
            })}\n\n`);
            res.end();
            return;
        }

        let fallbackMessage = "I'm sorry, I'm having trouble processing your query right now. Please try asking about specific supply chain batches or transit times.";
        try {
            const fallback = aiService?.getFallbackResponse?.(req?.body?.message || '');
