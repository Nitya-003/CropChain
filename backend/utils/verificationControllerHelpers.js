...
const consumeAndValidateChallenge = async ({
  ...
  if (!challengeRecord) {
    return {
      response: handleDuplicateIdempotencyKey(
        res,
        "Nonce is missing, expired, or already used",
      ),
    };
  }
  ...
  const normalizedReqWallet = walletAddress
    ? walletAddress.toLowerCase()
    : walletAddress;
  const normalizedStoredWallet = challengeRecord.walletAddress
    ? challengeRecord.walletAddress.toLowerCase()
    : challengeRecord.walletAddress;
  if (normalizedStoredWallet !== normalizedReqWallet) {
    return {
      response: handleDuplicateIdempotencyKey(
        res,
        "Challenge wallet address mismatch",
      ),
    };
  }
...
const handleChallengeValidation = (res, challengeRecord, requestPayload) => {
  ...
  if (
    ...
    return handleDuplicateIdempotencyKey(
      res,
      "Nonce does not match the active challenge",
    );
  }
