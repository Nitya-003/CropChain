...
class NotificationService {
   ...
   */
  alertRecall(batchId) {
    return this.log("alert", `🚨 ALERT: Recalled batch viewed: ${batchId}`, {
      batchId,
    });
  }
  ...
  async notifyBatchUpdated(batchId, stage, user, batch = null) {
    ...
    if (
      batch &&
      batch.farmerId &&
      batch.farmerId.toString() !== (user._id || user.id).toString()
    ) {
      await this.createInAppNotification(
        batch.farmerId,
        "Batch Stage Updated",
  ...
  async sendEmail(to, subject, body) {
    try {
      ...
      this.log("error", `Failed to queue email: ${subject}`, {
        to,
        subject,
        error: error.message,
      });
    ...
    if (result.fallback) {
      ...
      this.log("email", `Email sent synchronously: ${subject}`, {
        to,
        subject,
        messageId: result.messageId,
      });
    } else {
      this.log("error", `Email failed: ${subject}`, {
        to,
        subject,
        error: result.error,
      });
