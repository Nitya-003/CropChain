...
class BatchService {
  ...
  async createBatch(batchData, user, attempts = 3) {
    ...
    for (let i = 0; i < attempts; i++) {
      ...
      try {
        ...
        logger.info(
          `[SUCCESS] Batch created: ${batchId} by user ${user.id} (${user.email || "N/A"})`,
        );
        ...
        if (error.code === 11000 && i < attempts - 1) {
          logger.warn(
            `[RETRY] Duplicate batch ID detected. Retrying batch creation... (${i + 1}/${attempts})`,
          );
          continue;
        }
  ...
  toPublicBatch(batch) {
    const batchData =
      typeof batch.toJSON === "function" ? batch.toJSON() : batch;
  ...
  async updateBatch(batchId, updateData, user) {
    try {
      ...
      if (!isValidStage(normalizedStage)) {
        throw new Error(
          `Invalid stage: ${updateData.stage}. Must be one of: ${getStagesString()}`,
        );
      }
      ...
      logger.info(
        `[SUCCESS] Batch updated: ${batchId} to stage ${normalizedStage} by ${updateData.actor}`,
      );
  ...
  async recallBatch(batchId, adminUser) {
    try {
      ...
      logger.info(
        `🚨 RECALL by admin ${adminUser.email || "unknown"} for batch ${batchId}`,
      );
  ...
  async getAllBatches(limit = 100) {
    try {
      ...
      const [totalBatches, recalledBatches, recentBatches, batchStats] =
        await Promise.all([
  ...
  async getBatchByIdOrPartial(id) {
    ...
    batch = await Batch.findOne({
      batchId: { $regex: escRegex(cleanId), $options: "i" },
    });
    return batch;
  }
  ...
  async searchBatches(filters = {}) {
    ...
    if (filters.farmerName) {
      query.farmerName = {
        $regex: escRegex(filters.farmerName),
        $options: "i",
      };
    }
    if (filters.origin) {
  ...
  async syncToBlockchain(batch, action) {
    if (!blockchainService.isAvailable()) {
      logger.warn(
        `[BatchService] Blockchain not available. Skipping queueing for ${batch.batchId}`,
      );
      return;
    }
    ...
    try {
      if (action === "create") {
        ...
        logger.info(
          `[BatchService] Queued updateBatch job for ${batch.batchId}`,
        );
      }
    } catch (error) {
      logger.error(
        `Failed to queue blockchain job for batch ${batch.batchId}:`,
        error.message,
      );
    }
  }
