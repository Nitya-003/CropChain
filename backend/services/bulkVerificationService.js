...
const processJob = async (jobId, records, adminId, { dryRun } = {}) => {
  ...
  const BULK_JOB_CONCURRENCY =
    parseInt(process.env.BULK_JOB_CONCURRENCY, 10) || 10;
  const PROGRESS_UPDATE_EVERY =
    parseInt(process.env.BULK_JOB_PROGRESS_UPDATE_EVERY, 10) || 20;
  ...
  const processRow = async (i) => {
    ...
    const expiresAtVal = record.expiresat
      ? parseInt(record.expiresat, 10)
      : undefined;
    ...
    const action =
      CHALLENGE_ACTIONS[actionStr] || CHALLENGE_ACTIONS.ISSUE_CREDENTIAL;
    ...
    try {
      ...
      if (!reservation.reserved) {
        ...
        results[i] = {
          ...
          status,
          details: {
            message: "Request already in progress or duplicate idempotency key",
          },
          originalInput,
        };
        return;
      ...
      if (dryRun) {
        const predictedDetails = {
          predicted: true,
          message:
            "Dry-run prediction: would execute side effects if dryRun=false",
          willExecute: {
            action,
      ...
      if (signature) {
        if (!nonce || !expiresAtVal) {
          throw new Error(
            "Nonce and expiresAt are required when signature is provided",
          );
        }
        ...
        if (action === CHALLENGE_ACTIONS.ISSUE_CREDENTIAL) {
          ...
          outcome = await didService.issueCredential(
            userId,
            adminId,
            signature,
            walletAddress,
          ...
          outcome = await didService.linkWallet(
            userId,
            walletAddress,
            signature,
            challengeRecord,
  ...
  const concurrency = Math.max(
    1,
    Math.min(BULK_JOB_CONCURRENCY, records.length || 1),
  );
  ...
  const successCount = results.filter(
    (r) =>
      r &&
      ["success", "skipped", "dry-run-success", "dry-run-skipped"].includes(
        r.status,
      ),
  ).length;
