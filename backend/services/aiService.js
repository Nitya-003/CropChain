  ...
  constructor() {
    ...
    if (this.provider === "gemini") {
      try {
        ...
        logger.info(
          `✓ AI chatbot initialized using Google Gemini (${this.modelName})`,
        );
      } catch (error) {
        logger.error(
          "Failed to load Google Generative AI SDK, falling back to basic responses:",
          error.message,
        );
        this.provider = "fallback";
      }
      ...
      logger.warn(
        "OpenAI/Gemini API key not found. AI chatbot will use fallback responses.",
      );
    }
  }
...
Be helpful, friendly, and focus on CropChain-specific guidance. Use agricultural terminology appropriately.`;
    ...
    return [
      {
        ...
        function: {
          name: "search_batch",
          description:
            "Search for a specific crop batch by ID (supports full ID, partial ID, or ID with hashtag)",
          parameters: {
            type: "object",
            properties: {
              batchId: {
                type: "string",
                description:
                  "The batch ID or partial numeric ID to search for (e.g., CROP-2024-0001, BATCH000001, #1042, 1042)",
              },
            },
            required: ["batchId"],
          },
      ...
      },
      {
        ...
        function: {
          name: "search_batches",
          description:
            "Search for multiple crop batches matching specific filters (like crop type, farmer name, origin location, current stage, or status)",
          parameters: {
            type: "object",
            properties: {
              cropType: {
                type: "string",
                description:
                  "Optional crop type filter (e.g. rice, wheat, corn, tomato)",
              },
              farmerName: {
                type: "string",
              ...
              currentStage: {
                type: "string",
                description:
                  "Optional current stage: farmer, mandi, transport, retailer",
              },
              status: {
                type: "string",
      ...
      },
      {
        ...
        function: {
          ...
          description:
            "Retrieve the most recently registered crop batch, optionally filtered by crop type",
          parameters: {
            type: "object",
            properties: {
              cropType: {
                type: "string",
                description:
                  "Optional crop type to filter the latest batch (e.g. rice, wheat, corn, tomato)",
              },
            },
          },
        },
      },
      {
        ...
        function: {
          ...
          description:
            "Get overall dashboard statistics about batches in the system (total batches, total quantity, total farmers, recent activity)",
          parameters: {
            type: "object",
            properties: {},
            required: [],
      ...
      },
      {
        ...
        function: {
          ...
          description:
            "Explain a specific CropChain process or feature (e.g., batch creation, QR scanning, blockchain, lifecycle stages, transit tracking)",
          parameters: {
            type: "object",
            properties: {
              topic: {
      ...
      const convertSchema = (schema) => {
        ...
        if (newSchema.properties) {
          newSchema.properties = Object.fromEntries(
            Object.entries(newSchema.properties).map(([key, val]) => [
              key,
              convertSchema(val),
            ]),
          );
  ...
  async executeFunction(functionName, parameters, batchService) {
    try {
      switch (functionName) {
        case "search_batch":
          const batch = await batchService.getBatchByIdOrPartial(
            parameters.batchId,
          );
          if (batch) {
            return {
              ...
              data: {
                ...
                lifecycle: batch.lifecycle
                  ? {
                      ...
                      stageHistory: (batch.lifecycle.stageHistory || []).map(
                        (h) => ({
                          stage: h.stage,
              ...
              data: {
                ...
                lifecycle: latest.lifecycle
                  ? {
                      ...
                      stageHistory: (latest.lifecycle.stageHistory || []).map(
                        (h) => ({
                          stage: h.stage,
                          timestamp: h.timestamp,
                          updatedBy: h.updatedBy,
  ...
  getProcessExplanation(topic) {
    const explanations = {
      "batch creation": {
        success: true,
        explanation:
          'To create a batch: 1) Go to "Add Batch" page, 2) Fill in farmer details, crop type, quantity, and harvest date, 3) Add certifications if applicable, 4) Submit to generate a unique batch ID and QR code, 5) The batch is recorded on the blockchain for immutable tracking.',
      },
      "qr scanning": {
        success: true,
        explanation:
          "QR codes provide instant access to batch information. Consumers can scan the QR code on products to see the complete farm-to-fork journey, including farmer details, harvest date, and all supply chain updates.",
      },
      "supply chain": {
        success: true,
        explanation:
          "The supply chain has 4 stages: Farmer (harvest) → Mandi (processing/wholesale) → Transport (logistics) → Retailer (final sale). Each stage update is recorded with timestamp, location, and actor details for complete traceability.",
      },
      blockchain: {
        success: true,
        explanation:
          "Blockchain ensures data immutability - once recorded, information cannot be altered. This creates trust between all parties and prevents fraud in the supply chain. Each update gets a unique hash for verification.",
      },
      "immutable record": {
        success: true,
        explanation:
          "An immutable record means the data cannot be changed or deleted once written to the blockchain. This ensures the integrity of crop tracking information and builds trust among farmers, retailers, and consumers.",
      },
    };
  ...
  async getSmartFallbackResponse(message, batchService) {
    ...
    const numberMatch =
      ...
      lowerMessage.match(/batch\s+#?(\d+)/) ||
      lowerMessage.match(/#\s*(\d+)/);
    ...
    if (batchIdSearch && batchService) {
      try {
        ...
        if (batch) {
          ...
          const updatesText = (batch.updates || [])
            .map(
              (u) =>
          ...
          const lifecycleText =
            batch.lifecycle &&
            batch.lifecycle.stageHistory &&
            batch.lifecycle.stageHistory.length > 0
              ? "\n**Lifecycle History:**\n" +
                batch.lifecycle.stageHistory
                  .map(
                    (h) =>
          ...
          const spoiledWarning =
            batch.iotData && batch.iotData.isSpoiled
      ...
      try {
        ...
        if (batch) {
          const qualityPassed =
            batch.lifecycle &&
            batch.lifecycle.stageHistory &&
            batch.lifecycle.stageHistory.some(
              (h) => h.stage === "Quality Checked",
            );
...
You can view the full journey of this batch here: [View Journey](/batch/${batch.batchId}/journey)`,
    ...
    if (
      ...
      try {
        const batches = await batchService.searchBatches({
          cropType: foundCropType,
        });
        if (batches && batches.length > 0) {
          const listText = batches
            .map(
              (b) =>
...
Click on any batch ID to trace its journey.`,
    ...
    if (
      ...
      lowerMessage.includes("system statistics")
    ) {
      try {
        const stats = await batchService.getDashboardStats();
        return {
...
- **Batches Added Last 30 Days:** ${stats.stats.recentBatches}`,
  ...
  getFallbackResponse(message) {
    ...
    if (
      ...
      (lowerMessage.includes("track") || lowerMessage.includes("find"))
    ) {
      return {
        success: true,
        message:
          "To track a batch, you can either scan the QR code or search by batch ID (format: CROP-YYYY-XXXX) on the Track Batch page. This will show you the complete supply chain journey.",
      };
    }
    ...
    if (lowerMessage.includes("qr") || lowerMessage.includes("scan")) {
      return {
        success: true,
        message:
          "QR codes are generated automatically when you create a batch. Consumers can scan these codes to see the complete farm-to-fork journey of their products.",
      };
    }
    ...
    if (lowerMessage.includes("create") || lowerMessage.includes("add")) {
      return {
        success: true,
        message:
          "To create a new batch, go to the 'Add Batch' page and fill in the farmer details, crop information, and harvest date. The system will generate a unique batch ID and QR code.",
      };
    }
    ...
    if (
      ...
      return {
        success: true,
        message:
          "CropChain uses blockchain technology to create immutable records. Once data is recorded, it cannot be changed, ensuring transparency and trust in the supply chain.",
      };
    }
  }
  ...
  calculateTransitStats(batches, cropType) {
    ...
    batches.forEach((b) => {
      ...
      if (farmerUpdate && mandiUpdate) {
        farmerToMandi.push(
          new Date(mandiUpdate.timestamp) - new Date(farmerUpdate.timestamp),
        );
      }
      if (mandiUpdate && transportUpdate) {
        mandiToTransport.push(
          new Date(transportUpdate.timestamp) - new Date(mandiUpdate.timestamp),
        );
      }
      if (transportUpdate && retailerUpdate) {
        transportToRetailer.push(
          new Date(retailerUpdate.timestamp) -
            new Date(transportUpdate.timestamp),
        );
      }
      if (farmerUpdate && retailerUpdate) {
        totalTransit.push(
          new Date(retailerUpdate.timestamp) - new Date(farmerUpdate.timestamp),
        );
      }
    });
  ...
  async chatWithBatchContext(
    ...
    if (foundCropType && batchService) {
      ...
      try {
        const batchesOfCrop = await batchService.searchBatches({
          cropType: foundCropType,
        });
        if (batchesOfCrop && batchesOfCrop.length > 0) {
          cropTransitStats = this.calculateTransitStats(
            batchesOfCrop,
            foundCropType,
          );
        }
...
${pageContext ? `\n${pageContext}\nUse this to tailor generic questions (e.g. "what should I do here?") to what's relevant on that page, instead of a generic answer.\n` : ""}
...
5. If the context contains warnings (e.g. spoilage or recall), highlight them clearly using alert emojis.`;
    ...
    if (this.provider === "fallback") {
      ...
      const mentionsBatch =
        lowerMessage.includes("batch") || lowerMessage.includes("#");
      const mentionsStats =
        lowerMessage.includes("stat") ||
        lowerMessage.includes("dashboard") ||
        lowerMessage.includes("total") ||
      ...
      if (!mentionsCrop && !mentionsBatch && !mentionsStats) {
        finalMessage =
          "I'm sorry, but I can only answer questions related to the supply chain batch data provided in this context. I do not have access to that information.";
      }
...
Return the structured risk assessment JSON.`;
  ...
  calculateLocalSpoilageRisk(batch, transitHours) {
    ...
    if (temp !== undefined && temp !== null && temp > maxTemp) {
      factors.push(
        `Temperature (${temp}°F) exceeded optimal threshold (${maxTemp}°F) for ${crop}`,
      );
      riskScore = Math.max(riskScore, 75);
    }
    if (humidity !== undefined && humidity !== null && humidity > maxHumidity) {
      factors.push(
        `Humidity (${humidity}%) exceeded optimal threshold (${maxHumidity}%) for ${crop}`,
      );
    ...
    if (crop === "tomato") {
      if (transitHours > 72) {
        ...
        factors.push(
          `Transit time (${transitHours.toFixed(1)} hrs) exceeds 72 hours for highly perishable tomato`,
        );
      } else if (transitHours > 36) {
        riskScore = Math.max(riskScore, 50);
        factors.push(
          `Transit time (${transitHours.toFixed(1)} hrs) exceeds 36 hours for tomato`,
        );
      } else {
        riskScore = Math.max(riskScore, 15);
        factors.push(
          `Transit time (${transitHours.toFixed(1)} hrs) is within safe limits for tomato`,
        );
      }
    } else if (crop === "corn") {
      if (transitHours > 96) {
        ...
        factors.push(
          `Transit time (${transitHours.toFixed(1)} hrs) exceeds 96 hours for semi-perishable corn`,
        );
      } else if (transitHours > 48) {
        riskScore = Math.max(riskScore, 45);
        factors.push(
          `Transit time (${transitHours.toFixed(1)} hrs) exceeds 48 hours for corn`,
        );
      } else {
        riskScore = Math.max(riskScore, 10);
        factors.push(
          `Transit time (${transitHours.toFixed(1)} hrs) is within safe limits for corn`,
        );
      }
    } else {
      // Rice or Wheat
      if (transitHours > 720) {
        ...
        factors.push(
          `Transit time (${transitHours.toFixed(1)} hrs) exceeds 30 days for ${crop}`,
        );
      } else if (transitHours > 360) {
        // 15 days
        riskScore = Math.max(riskScore, 30);
        factors.push(
          `Transit time (${transitHours.toFixed(1)} hrs) exceeds 15 days for ${crop}`,
        );
      } else {
        riskScore = Math.max(riskScore, 5);
        factors.push(
          `Transit time (${transitHours.toFixed(1)} hrs) is within safe limits for ${crop}`,
        );
      }
    }
