...
class DIDService {
  ...
  async issueCredential(
    ...
    try {
      ...
      if (!verifier || !isAdminRole(verifier.role)) {
        try {
          await VerificationEvent.create({
            ...
            status: "failure",
            metadata: {
              error: "Only Mandi officers (admins) can verify users",
            },
          });
        } catch (e) {
          console.error(e);
      ...
      if (
        !challenge ||
        challenge.action !== CHALLENGE_ACTIONS.ISSUE_CREDENTIAL
      ) {
        try {
          await VerificationEvent.create({
            action: "credential_issued",
  ...
  async checkVerificationStatus(userId) {
    try {
      ...
      return {
        ...
        role: user.role,
        verifiedAt: user.verification?.verifiedAt,
        credentialHash: user.verification?.isVerified
          ? user.verification?.credentialHash
          : null,
      };
    } catch (error) {
      throw error;
