  ...
  async generateBatchJourneyPDF(batch) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const addSectionTitle = (text) => {
        ...
        doc.moveDown(0.3);
        doc.fillColor('#d1d5db').rect(50, y + 2, pageWidth, 1).fill();
        y = doc.y + 12;
      };
      ...
      const fields = [
        ...
        ['Harvest Date', batch.harvestDate ? new Date(batch.harvestDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : 'N/A'],
        ['Origin', batch.origin],
      ...
      if (batch.qrCode) {
        try {
          ...
          if (match) {
            doc.image(Buffer.from(match[1], 'base64'), 50, y, { width: 100, height: 100 });
      ...
      addLabelValue('Created At', batch.createdAt ? new Date(batch.createdAt).toLocaleString() : 'N/A');
      addLabelValue('Updated At', batch.updatedAt ? new Date(batch.updatedAt).toLocaleString() : 'N/A');
      y += 6;
      ...
      if (updates.length === 0) {
        ...
        updates.forEach((update, index) => {
          ...
          const color = stageColors[update.stage] || { bg: '#f3f4f6', text: '#374151' };

          // Background card
          doc.fillColor('#f9fafb').roundedRect(50, y, pageWidth, estimatedHeight - 8, 4).fill();

          // Stage circle indicator
          doc.fillColor(color.text).fontSize(10).font('Helvetica-Bold');
          doc.circle(70, y + 12, 8).fill(color.bg).fill(color.text);
          doc.text(`${index + 1}`, 66, y + 7, { width: 10, align: 'center' });

          // Stage name
          doc.fillColor(color.text).fontSize(10).font('Helvetica-Bold');
          doc.text(update.stage.charAt(0).toUpperCase() + update.stage.slice(1), 88, y + 2);
          ...
          doc.fillColor('#6b7280').fontSize(8).font('Helvetica');
          const ts = update.timestamp ? new Date(update.timestamp).toLocaleString() : '';
          doc.text(ts, 50 + pageWidth - 180, y + 2, { width: 175, align: 'right' });
