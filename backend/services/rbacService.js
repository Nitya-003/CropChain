...
class RBACService {
  ...
  static checkAllPermissions(user, permissions) {
    if (!user || !user.role) return false;
    const rolePerms = require("../constants/permissions").getRolePermissions(
      user.role,
    );
  ...
  static canUserApproveAction(user, action) {
    ...
    if (
      user.role !== ROLES.QUALITY_INSPECTOR &&
      user.role !== ROLES.SUPER_ADMIN
    ) {
      return {
        canApprove: false,
        reason: "Only Quality Inspectors can approve this action",
      };
    }
    if (user.role === ROLES.QUALITY_INSPECTOR) {
      if (!user.inspectorCredentials)
        return { canApprove: false, reason: "Inspector credentials not found" };
      if (!user.inspectorCredentials.isCertified)
        return { canApprove: false, reason: "Inspector is not certified" };
      if (!user.inspectorCredentials.isActive)
        return { canApprove: false, reason: "Inspector is not active" };
      if (
        user.inspectorCredentials.expiresAt &&
        new Date() > user.inspectorCredentials.expiresAt
      ) {
        return {
          canApprove: false,
          reason: "Inspector certification has expired",
        };
      }
      if (user.status !== "active")
        return { canApprove: false, reason: "User account is not active" };
    }
  ...
  static canInitiateMultisigAction(user, action) {
    if (!user || !user.role)
      return { canInitiate: false, reason: "User not authenticated" };
    if (user.role === ROLES.ADMIN || user.role === ROLES.SUPER_ADMIN) {
      return { canInitiate: true, reason: "Authorized as admin" };
    }
    if (user.role === ROLES.QUALITY_INSPECTOR) {
      if (user.canApproveMultisig)
        return { canInitiate: true, reason: "Authorized as inspector" };
      return { canInitiate: false, reason: "Inspector not properly certified" };
    }
    return {
      canInitiate: false,
      reason: "Not authorized to initiate this action",
    };
  }
  ...
  static canAccessBatch(user, batch, accessType = "read") {
    ...
    const permissionMap = {
      read: PERMISSIONS.BATCH_READ,
      update: PERMISSIONS.BATCH_UPDATE,
      delete: PERMISSIONS.BATCH_DELETE,
    };
    const requiredPermission = permissionMap[accessType];
    if (!requiredPermission)
      return { canAccess: false, reason: "Invalid access type" };
  ...
  static canUpdateStage(user, stage) {
    ...
    if (user.role === ROLES.ADMIN || user.role === ROLES.SUPER_ADMIN)
      return { canUpdate: true, reason: "Admin access" };
    const stagePermissionMap = {
      farmer: PERMISSIONS.STAGE_FARMER,
      mandi: PERMISSIONS.STAGE_MANDI,
      transport: PERMISSIONS.STAGE_TRANSPORT,
      retailer: PERMISSIONS.STAGE_RETAILER,
    };
    const requiredPermission = stagePermissionMap[stage?.toLowerCase()];
    if (!requiredPermission)
      return { canUpdate: false, reason: "Invalid stage" };
    if (!this.checkPermission(user, requiredPermission))
      return { canUpdate: false, reason: `Not authorized for stage: ${stage}` };
  ...
  static canManageUser(manager, target) {
    if (!manager || !target)
      return { canManage: false, reason: "Invalid users" };
    ...
    if (manager.role === ROLES.SUPER_ADMIN) {
      if (target.role === ROLES.SUPER_ADMIN)
        return { canManage: false, reason: "Cannot manage other super admins" };
  ...
  static requirePermissions(permissions, options = {}) {
    ...
    return (req, res, next) => {
      ...
      const hasAuth = requireAll
        ? this.checkAllPermissions(req.user, permArray)
        : this.checkAnyPermission(req.user, permArray);
      if (!hasAuth) {
        logger.warn(
          `[RBAC] Permission denied for user ${req.user.email}: needs ${permArray.join(", ")}`,
        );
        return res
          .status(403)
          .json({
            error: "Forbidden",
            message: `Missing required permission(s): ${permArray.join(", ")}`,
          });
      }
      next();
    };
