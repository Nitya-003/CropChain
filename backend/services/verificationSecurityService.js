...
const CHALLENGE_TTL_SECONDS =
  parseInt(process.env.VERIFICATION_CHALLENGE_TTL_SECONDS, 10) || 180;
const IDEMPOTENCY_TTL_SECONDS =
  parseInt(process.env.VERIFICATION_IDEMPOTENCY_TTL_SECONDS, 10) ||
  24 * 60 * 60;
...
const buildChallengeKey = ({ action, actorId, nonce }) =>
  `verification:challenge:${action}:${actorId}:${nonce}`;
const buildIdempotencyKey = ({ action, actorId, key }) =>
  `verification:idempotency:${action}:${actorId}:${key}`;
...
const createChallenge = async ({ action, actorId, userId, walletAddress }) => {
  ...
  await redis.set(
    buildChallengeKey({ action, actorId, nonce }),
...
const getIdempotencyRecord = async ({ action, actorId, key }) => {
  ...
  const rawRecord = await redis.get(
    buildIdempotencyKey({ action, actorId, key }),
  );
...
const storeCompletedIdempotencyRecord = async ({
  action,
  actorId,
  key,
  fingerprint,
