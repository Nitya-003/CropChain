...
const runSpoilageRiskAssessment = async () => {
    try {
        ...
        logger.info(`Found ${activeTransitBatches.length} active transit batches for spoilage risk analysis.`);

        for (const batch of activeTransitBatches) {
            // Find when it entered 'transport' stage
            const transportUpdate = batch.updates.find(u => u.stage === 'transport');
            const transportTime = transportUpdate ? transportUpdate.timestamp : batch.updatedAt || batch.createdAt;
            
            const elapsedMs = Date.now() - new Date(transportTime).getTime();
            const transitHours = elapsedMs / (1000 * 60 * 60);

            logger.info(`Analyzing batch ${batch.batchId} (${batch.cropType}) - transit time: ${transitHours.toFixed(2)} hours`);

            const riskAssessment = await aiService.predictSpoilageRisk(batch, transitHours);
            ...
            logger.info(`Updated batch ${batch.batchId} spoilage risk: ${riskAssessment.riskLevel} (${riskAssessment.riskScore}%)`);
        ...
        logger.error('Error in background spoilage risk assessment agent:', { error: error.message, stack: error.stack });
    }
};
