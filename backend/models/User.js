...
const userSchema = new mongoose.Schema(
  {
    ...
    inspectorCredentials: {
      ...
      specializations: [
        {
          type: String,
          enum: ["contamination", "quality", "safety", "organic", "all"],
        },
      ],
...
userSchema.index(
  { "inspectorCredentials.certificationId": 1 },
  { sparse: true },
);
...
userSchema.virtual("canApproveMultisig").get(function () {
  ...
  if (!this.inspectorCredentials || !this.inspectorCredentials.isCertified)
    return false;
  if (!this.inspectorCredentials.isActive) return false;
  if (
    this.inspectorCredentials.expiresAt &&
    new Date() > this.inspectorCredentials.expiresAt
  )
    return false;
  return true;
});
...
userSchema.methods.hasPermission = function (permission) {
  const {
    hasPermission: checkRolePermission,
