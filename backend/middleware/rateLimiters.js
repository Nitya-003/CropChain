...
const registerWindowMs =
  parseInt(process.env.REGISTER_RATE_LIMIT_WINDOW_MS, 10) || 60 * 60 * 1000; // 1 hour
const registerLimiter = rateLimit({
  windowMs: registerWindowMs,
  max: isTestEnv
    ? 10000
    : parseInt(process.env.REGISTER_RATE_LIMIT_MAX, 10) || 3,
  message: {
    error:
...
const CHALLENGE_WINDOW_MS =
  parseInt(process.env.CHALLENGE_RATE_LIMIT_WINDOW_MS, 10) || 10 * 60 * 1000; // 10 minutes
const CHALLENGE_USER_MAX =
  parseInt(process.env.CHALLENGE_RATE_LIMIT_USER_MAX, 10) || 10; // 10 / 10 min per user
const CHALLENGE_IP_MAX =
  parseInt(process.env.CHALLENGE_RATE_LIMIT_IP_MAX, 10) || 5; // 5 / 10 min per IP

// Credential issuance stricter limits
const CREDENTIAL_WINDOW_MS =
  parseInt(process.env.CREDENTIAL_RATE_LIMIT_WINDOW_MS, 10) || 10 * 60 * 1000;
const CREDENTIAL_USER_MAX =
  parseInt(process.env.CREDENTIAL_RATE_LIMIT_USER_MAX, 10) || 5;
const CREDENTIAL_IP_MAX =
  parseInt(process.env.CREDENTIAL_RATE_LIMIT_IP_MAX, 10) || 2;

// Bulk CSV job initiation (admin) - jobs per hour per admin (and per IP)
const BULK_WINDOW_MS =
  parseInt(process.env.BULK_JOB_RATE_LIMIT_WINDOW_MS, 10) || 60 * 60 * 1000; // 1 hour
const BULK_ADMIN_MAX =
  parseInt(process.env.BULK_JOB_RATE_LIMIT_ADMIN_MAX, 10) || 2; // 2 jobs / hour per admin
