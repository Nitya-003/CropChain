...
const protect = async (req, res, next) => {
  try {
    if (
      ...
      return res
        .status(401)
        .json({ error: "Not authorized", message: "No token provided" });
    }
    ...
    if (error.name === "JsonWebTokenError") {
      return res
        .status(401)
        .json({ error: "Not authorized", message: "Invalid token" });
    }
    return res
      .status(500)
      .json({ error: "Server Error", message: "Authentication check failed" });
  }
};
...
const verifiedOnly = (req, res, next) => {
  ...
  return res
    .status(403)
    .json({ error: "Access denied", message: "Verified credential required" });
};
...
const authorizeBatchOwner = async (req, res, next) => {
  try {
    ...
    if (!batchId) {
      return res
        .status(400)
        .json({ error: "Bad Request", message: "Batch ID is required" });
    }
    ...
    if (!requiredPermission) {
      return res
        ...
        .json({
          error: "Access denied",
          message: "Action not allowed on batches",
        });
    }
    ...
    const batchFarmerIdStr =
      batch.farmerId?.toString().trim().toLowerCase() || "";
    ...
    const isOwner =
      batchFarmerIdStr === userFarmerIdStr || batchFarmerIdStr === userIdStr;

    // Check if the user's role has explicit permission for the action (independent of ownership)
    const hasPermission = hasExplicitRolePermission(
      req.user.role,
      requiredPermission,
    );
    ...
    if (!isOwner && !hasPermission) {
      logger.warn("Auth fail: unauthorized batch access attempt", {
        userId,
        role: req.user.role,
        method: req.method,
        batchId,
...
const authorizeStageTransition = (req, res, next) => {
  ...
  if (!allowedStages || !allowedStages.includes(stage)) {
    ...
    return res
      ...
      .json({
        error: "Access denied",
        message: `Role '${userRole}' is not authorized to update stage '${stage}'`,
      });
  }
  next();
};
...
const authorizeBlockchainTransaction = (req, res, next) => {
  ...
  if (!blockchainAllowedRoles.includes(req.user.role)) {
    logger.warn("Blockchain transaction violation", {
      email: req.user.email,
      role: req.user.role,
    });
    return res
...
const inspectorOnly = async (req, res, next) => {
  if (!req.user)
    return res
      ...
      .json({ error: "Not authorized", message: "Authentication required" });
  if (
    req.user.role !== ROLES.QUALITY_INSPECTOR &&
    ...
    return res
      ...
      .json({
        error: "Access denied",
        message: "Quality Inspector access required",
      });
  }
  if (
    req.user.role === ROLES.QUALITY_INSPECTOR &&
    !req.user.canApproveMultisig
  ) {
    return res
      .status(403)
      .json({
        error: "Access denied",
        message: "Inspector certification is not valid, active, or has expired",
      });
  }
  next();
};
  ...
  return async (req, res, next) => {
    ...
    if (!RBACService.requiresMultisigApproval(actionType)) {
      return res
        ...
        .json({
          ...
          message: "Action does not require multisig approval",
        });
    }
    ...
    if (!canInitiate) {
      return res
        ...
        .json({
          error: "Access denied",
          message: `Not authorized to initiate this action: ${reason}`,
        });
    }
    next();
  };
  ...
  try {
    ...
    if (batch.safetyStatus && batch.safetyStatus !== "safe") {
      if (!isAdminRole(req.user.role)) {
        return res
          ...
          .json({
            error: "Access denied",
            message: `Batch is currently ${batch.safetyStatus} and cannot be modified`,
          });
      }
    }
    ...
    if (batch.hasPendingApproval && batch.hasPendingApproval()) {
      return res
        ...
        .json({
          error: "Access denied",
          message: "Batch has a pending approval request",
        });
    }
    ...
    logger.error("Safety status check error", {
      error: error.message,
      stack: error.stack,
    });
    return res
      .status(500)
      .json({ error: "Server error", message: "Failed to check batch status" });
  }
};
