...
module.exports = (app) => {
    ...
    if (process.env.NODE_ENV === 'development') allowedOrigins.push('http://localhost:3000', 'http://localhost:5173');
    ...
    const trustedHosts = (() => {
        ...
        if (process.env.FRONTEND_URL) {
            try { const hostname = new URL(process.env.FRONTEND_URL).hostname; if (hostname) hosts.add(hostname); } catch { }
        }
        if (process.env.ALLOWED_ORIGINS) {
            process.env.ALLOWED_ORIGINS.split(',').forEach(origin => {
                try { const hostname = new URL(origin.trim()).hostname; if (hostname) hosts.add(hostname); } catch { }
            });
        }
    ...
    app.use((req, res, next) => {
        ...
        if (host && !trustedHosts.has(host)) {
            ...
            return res.status(400).json({ error: 'Invalid request', code: 'INVALID_HOST' });
        }
        next();
    });
};
