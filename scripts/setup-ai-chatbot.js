...
async function setupAIChatbot() {
  ...
  if (hasApiKey.toLowerCase() === "y" || hasApiKey.toLowerCase() === "yes") {
    ...
    if (apiKey.trim()) {
      ...
      console.log(
        "⚠️  No API key provided. Chatbot will run in fallback mode.",
      );
    }
  } else {
    console.log(
      "⚠️  No API key configured. Chatbot will run in fallback mode.",
    );
    console.log(
      "   Fallback mode provides basic responses without AI intelligence.",
    );
  }
